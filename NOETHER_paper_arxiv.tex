...
%% ---- Abstract ----
\begin{abstract}
\textbf{Context.} Metamorphic Testing is recognised in the IEEE/ISO software-testing standards and increasingly recommended for AI systems, yet its progress is bottlenecked by metamorphic relation (MR) identification, which remains inductive and leaves three foundational questions open: \emph{origin}, \emph{closure}, and \emph{transferability}.

\textbf{Objective.} We treat the MR set of a mathematical-physics program family as a computable function of its operator algebra, and we make the limits of that derivation a theorem rather than a caveat.

\textbf{Method.} We introduce \textbf{NOETHER}, with an upstream empirical layer that curates a program-induced operator algebra and its decomposition over recurrent structures (symmetry, order, self-adjoint, time-reversal, limit, qualitative-dynamics, method-comparison, relational equivalence), and a downstream deductive layer, \texttt{CONSTRUCT-MP}, that derives a MetaPattern set with one pattern per operator-invariant equivalence class. A well-formedness result (algebraic closure under the \texttt{Translate} operator, Theorem~1; polynomial-time decidability under a finite generating set, Theorem~2) guarantees the construction drops no derivable MR. The substantive result is the \emph{Invariance-Blindness Theorem}: for the symmetry and self-adjoint blocks, the detection kernel of an algebra-derived MR equals exactly the faults that preserve the structure the MR exploits, a characterization attained by a finite test (faithfulness). We instantiate NOETHER on three operator-algebraic domains and on a home-field benchmark spanning thermal, fluid, and reactor physics.

\textbf{Results.} The block decomposition is a program-family invariant predictable before any test is run: reversible systems populate the time-reversal block and dissipative ones leave it empty. The Invariance-Blindness Theorem is supported empirically: symmetry MRs miss precisely the symmetry-preserving faults, while a neutral cross-implementation differential oracle carries the complementary common-mode blind spot, so the two oracles cross rather than nest and their union strictly improves on either alone. The strictly stronger absolute-completeness conjecture (Theorem~$1'$) is falsified on the PWR core algebra $\mathcal{A}_{\mathrm{PWR}}$ by two independent counterexamples that characterize the obstruction as five pairwise-independent \texttt{Translate}-extension dimensions.

\textbf{Conclusion.} For these families an MR set is algebraically derivable with a precisely characterized blind set; NOETHER lifts induction from per-program MR sampling to a per-domain algebraic layer, and states explicitly where its derivation is tight (the symmetry and self-adjoint blocks, within the linear operator-implementation fault class) and where it is not.
\end{abstract}
...
Four further pattern-catalogue or MR-grammar candidates raised in peer review (Hu et~al.\ 2019; Mariani 2018; Liu et~al.\ 2020; Lin 2020) could not be located through the standard fallback chain (CrossRef, OpenAlex, DBLP, Semantic Scholar, Google Scholar) under the citation profiles provided, and are therefore not cited; the closest verifiable cousins in this space are Zhou et al.~\cite{Zhou2020SymmetryMRP} and Ying et al.~\cite{Ying2025MRPatterns}.
...
\begin{table}[h]
\centering
\caption{NOETHER structural scope analysis on Sun et al.~2021 METRIC$+$
...
\paragraph{Out-of-construction transferability (held-out).} To test transferability beyond the domains used to curate the block decomposition, the frozen blocks were applied without re-fitting to program families outside the construction set. (i) \emph{Industrial reactor codes.} The 110 expert-approved metamorphic relations of three production codes, BAMBOO-C's SPARK (core depletion) and LOCUST (lattice), and the SACOS sub-channel thermal-hydraulics code, are subsumed without exception by the order ($O_{\le}$) block (Wilson 95\% interval on subsumption $[0.966, 1.000]$), with no orphan requiring a ninth block and a further 18 valid relations beyond the initial expert sets falling in the same block. This is a code-level held-out confirmation, but it exercises a single block and so corroborates transferability of $O_{\le}$ rather than of the full decomposition. (ii) \emph{Non-physics libraries.} On dense numerical linear algebra (tested on \texttt{numpy.linalg}) and digital signal processing (tested on \texttt{numpy.fft}), both outside the curation set, the algebra-derived relations populate six blocks each ($G$, $T^{*}$, $O_{\le}$, $\mathcal{L}^{*}$, $\mathcal{E}^{*}$, and a conservation invariant), every relation confirmed executably on the independent library, and the two FA-rank-tight blocks behave as Theorem~\ref{thm:closure}'s companion Invariance-Blindness result predicts (the permutation/orthogonal $G$ block and the self-adjoint $T^{*}$ block attain the tight kernel under a finite generating set). The two legs are complementary: the industrial corpus supplies expert-validated breadth on a single block in domain, while the library instantiations exercise the multi-block structure and the tight blocks out of domain. Corpora, derivations, and executable checks are in supplementary material \texttt{S11\_n5\_industrial} and \texttt{S12\_n5\_crossdomain}.

\paragraph{Expert monotonicity bias and the coverage argument.} A regularity across the three production codes is that all 110 expert-approved relations are order-block (monotone sensitivity) relations: increasing one input moves an output in a fixed direction. This is consistent with how MR identification proceeds in practice, from parameter-sensitivity intuition, which is exactly the order block; the symmetry, self-adjoint, conservation, limit, and method-comparison blocks require explicit operator-algebraic recognition and are systematically under-sampled. Yet the governing equations of these codes contain precisely those structures: SACOS solves two-fluid mass, momentum, and energy conservation; LOCUST defines its homogenisation by reaction-rate conservation and its lattice by a geometric symmetry group; SPARK is a $k$-eigenvalue neutron balance with a critical-boron fixed point. NOETHER derives the corresponding conservation, reciprocity, and symmetry relations directly from those equations, without expert input (supplementary \texttt{S11}). The detection consequence is quantified on the open home-field SUTs, where MRs carry block labels and faults are executable: an order-only battery, standing in for expert practice, detects 17 of 47 real faults, whereas the full algebra-derived battery detects 29 of 47, and the 12 faults ($25.5\%$) caught only by non-order blocks are exactly the kernel that the Invariance-Blindness result attributes to a single-block oracle. The industrial cross-block relations are equation-grounded and committed for executable validation on the production codes; the quantified gap is reported on the home-field substrate where execution is open.
