...
\paragraph{The MR-family layer.} A generator is instantiated as one or more executable \emph{MR families}, the unit a test engineer enumerates and the row of the empirical coverage matrix (\S\ref{para:real-bug-protocol}). Table~\ref{tab:mr-families} lists the ten families used in this paper. The map from five generators to ten families is one-to-many and program-family-dependent; which families are non-empty is determined by $\mathcal{A}_P$. Each family instantiates a block's MetaPattern from $\mathbb{M}(\mathcal{A}_P)$ under a fixed $\pi$-template, and a single MetaPattern may yield several families (the $G$-block MetaPattern $m_{\mathrm{inv}}$ yields both the equivariance family and, by the Noether correspondence of \S\ref{subsec:decomposition}, the conservation family), consistent with Step~3 of CONSTRUCT-MP and Remark~\ref{rem:single-class-instances}. The chain is therefore single, $\mathcal{D}(\mathcal{A}_P)$ blocks (five generators, two derived, one extension) $\to$ MetaPatterns $\mathbb{M}(\mathcal{A}_P)$ $\to$ MR families $\to$ MR instances; the family layer refines the existing MetaPattern set rather than introducing a parallel taxonomy.
