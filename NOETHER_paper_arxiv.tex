...
%% ---- Title and metadata ----
\title{NOETHER: A Constructive Framework for Metamorphic Pattern Identification from Operator Algebras}
...
\label{thm:decidable}
Suppose $\mathcal{A}_P$ admits a finite generating set $\mathrm{gen}(\mathcal{A}_P)$ of cardinality $n$, with each generator's invariant computation \emph{assumed to terminate} in time $t_i$. Then $\mathbb{M}(\mathcal{A}_P)$ is computable in time $O\!\bigl(\,n \cdot \max_i t_i \cdot \log n\,\bigr)$. This is a complexity bound on the construction rather than a decidability result: termination of per-block invariant extraction is assumed within $t_i$, and for the relational block $\mathcal{B}^{*}_{\mathrm{rel}}$ general query equivalence is undecidable, so only the fixed-rule-set fragment is covered (Remark~\ref{rem:decidable-brel}).
\end{theorem}
