...
The model has two layers. Layer~1 (L1) is the set of \emph{MetaPatterns}: five canonical structures on the solution space, each the program-level image of one algebraic structure (a group action, an order cone, an inner-product pairing, a one-parameter evolution, and a refinement topology). Layer~2 (L2) is the set of \emph{MR families}: the ten executable relations a test engineer enumerates, each instantiating one MetaPattern. Making the two layers explicit controls both how the decomposition is instantiated as executable tests and how detection independence is read.
