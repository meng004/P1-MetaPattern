...
\begin{remark}[From per-invariant MRs to block MetaPatterns]
\label{rem:ibt-mp}
\texttt{CONSTRUCT-MP} aggregates per block: a MetaPattern $m_s$ is the union of the
MR families of all invariants in block $s$ (Step~3). Its kernel is the intersection
$\ker(m_s)=\bigcap_{\iota}\ker(\rho_{\iota,s})$ (the aggregate detects a fault iff some
constituent MR does). By Theorem~\ref{thm:ibt},
$\ker(m_s)=\{L : P_L \text{ preserves the full structure } T_s\}$. The theorem thus
operates at the per-invariant level while remaining consistent with the coarser
block-level MetaPattern granularity, so no re-counting of MetaPatterns is required.
\end{remark}
