...
symmetry ($G$) and self-adjoint ($T^{*}$) blocks. Within the linear
operator-implementation fault class $\Theta=\mathbb{R}^{N\times N}$, an
algebra-derived MR's detection kernel equals exactly the structure-preserving
faults $\mathcal{C}(T_s)$: a fault survives the MR if and only if it preserves
the structure $T_s$ the MR exploits. The characterisation is faithfulness-tight,
attained by a finite witness set (Lemma~\ref{lem:reachability}). A neutral
cross-implementation differential oracle is the complementary common-mode oracle
that covers part of the structure-preserving blind set.}
\label{fig:ibt}
\end{figure}
...
Remark~\ref{rem:single-class-instances} treats a non-empty block $s$ as a single
$\sim_s$ class, $m_{s,[\iota]}$ coincides with the block-level MetaPattern reported
