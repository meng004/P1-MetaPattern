...
%% corollary*. Cross-refs are resolved through the main manuscript.
...
Let structural component $s\in\mathcal{D}(\mathcal{A}_P)$ carry a structure $T_s$ (for $s=G$, a group
...
exact blind set. We now characterize that set (Figure~\ref{fig:ibt}).

\begin{theorem}[Invariance-Blindness, $G$ and $T^{*}$ MetaPatterns]
\label{thm:ibt}
...
%% Detection kernel = structure-preserving faults (blind spot)
...
%% Complementary differential oracle
...
\caption{The Invariance-Blindness Theorem (Theorem~\ref{thm:ibt}) for the
$G$ and $T^{*}$ MetaPatterns: within the linear
fault class $\Theta=\mathbb{R}^{N\times N}$ an algebra-derived MR's detection
kernel equals exactly the structure-preserving faults $\mathcal{C}(T_s)$
(faithfulness-tight, Lemma~\ref{lem:reachability}), with a neutral
cross-implementation differential oracle as the complementary common-mode
oracle covering the blind set.}
\label{fig:ibt}
\end{figure}

\begin{remark}[Per-MetaPattern granularity]
\label{rem:ibt-mp}
Under Step~3 of \texttt{CONSTRUCT-MP}, each MetaPattern $m_{s,[\iota]}$ corresponds
to a single $\sim_s$-equivalence class of invariants; the per-invariant statement
of Theorem~\ref{thm:ibt} therefore applies \emph{directly} to each $m_{s,[\iota]}$
without aggregation. When the per-instance grouping of
Remark~\ref{rem:single-class-instances} treats a non-empty structural component $s$ as a single
$\sim_s$ class, $m_{s,[\iota]}$ coincides with the MetaPattern reported
in the empirical sections.
\end{remark}

\begin{remark}[Worked instance: advection--diffusion, translation sub-class]
...
the secondary executability sanity check discussed in
...
input basis is unisolvent) is reported in supplementary S10.
\end{remark}
...
\begin{corollary*}[Single-MetaPattern incompleteness]
For any structural component $s$ with $\mathcal{C}(T_s)\neq\{L^{*}\}$ (a nontrivial structure-
preserving fault exists, noting $\mathcal{C}(T_s)\supseteq\{L^{*}\}$ always), the
single-MetaPattern battery $\{\rho_{\iota,s}\}_\iota$ is
...
structural kernels intersect trivially, not additional MRs of the same MetaPattern.
\end{corollary*}
...
regime is outside the identification claim and is treated as a limitation in
\S\ref{sec:threats-limitations}.
...
for $\mathcal{L}^{*}$ (Richardson ratio of norms); for these MetaPatterns only the sufficient
