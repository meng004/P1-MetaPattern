...
/** Auto-generated head-to-head MR class for SUT au_pow_int (org.apache.commons.math3.util.ArithmeticUtils.pow). Blocks: Lstar,Istar. */
public class T_au_pow_int {
    ...
    @Test public void nN_expsum() { for (int[] xy : POWI) { int k=xy[0],e=xy[1]; if (e<0) continue; long lhs=(long)org.apache.commons.math3.util.ArithmeticUtils.pow(k,e+1); long rhs=(long)org.apache.commons.math3.util.ArithmeticUtils.pow(k,e)*(long)k; if (Math.abs(lhs)>Integer.MAX_VALUE||Math.abs(rhs)>Integer.MAX_VALUE) continue; assertEquals(lhs,rhs); } }
    @Test public void nN_base0_1() { for (int e=0;e<6;e++){ assertEquals(1, org.apache.commons.math3.util.ArithmeticUtils.pow(1,e)); } assertEquals(1,org.apache.commons.math3.util.ArithmeticUtils.pow(7,0)); }
    @Test public void nN_prodbase() { for (int[] xy : POWI) { int k=xy[0],e=xy[1]; if (e<0||e>6) continue; long lhs=(long)org.apache.commons.math3.util.ArithmeticUtils.pow(k,e)*(long)org.apache.commons.math3.util.ArithmeticUtils.pow(2,e); long rhs=(long)org.apache.commons.math3.util.ArithmeticUtils.pow(k*2,e); if (Math.abs(lhs)>Integer.MAX_VALUE||Math.abs(rhs)>Integer.MAX_VALUE) continue; assertEquals(lhs,rhs); } }
    @Test public void bB_base0_1() { assertEquals(1,org.apache.commons.math3.util.ArithmeticUtils.pow(7,0)); assertEquals(1,org.apache.commons.math3.util.ArithmeticUtils.pow(1,5)); }
    @Test public void mM_scale_homog() { for (int[] xy : POWI) { int k=xy[0],e=xy[1]; if (e<0||e>6) continue; long lhs=(long)org.apache.commons.math3.util.ArithmeticUtils.pow(k,e)*(long)org.apache.commons.math3.util.ArithmeticUtils.pow(2,e); long rhs=(long)org.apache.commons.math3.util.ArithmeticUtils.pow(k*2,e); if (Math.abs(lhs)>Integer.MAX_VALUE||Math.abs(rhs)>Integer.MAX_VALUE) continue; assertEquals(lhs,rhs); } }
}