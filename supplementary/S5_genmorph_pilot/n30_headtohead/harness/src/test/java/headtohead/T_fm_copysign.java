...
/** Auto-generated head-to-head MR class for SUT fm_copysign (org.apache.commons.math3.util.FastMath.copySign). Blocks: Lstar,Istar. */
public class T_fm_copysign {
    ...
    @Test public void nN_magnitude() { for (double[] xy : DD) { assertEquals(Math.abs(xy[0]), Math.abs(org.apache.commons.math3.util.FastMath.copySign(xy[0],xy[1])), 1e-12); } }
    @Test public void nN_signfollow() { for (double[] xy : DD) { double y=xy[1]; if (y==0.0) continue; double r=org.apache.commons.math3.util.FastMath.copySign(xy[0],y); if (xy[0]!=0.0) assertTrue((r>=0.0)==(y>=0.0)); } }
    @Test public void nN_idem() { for (double[] xy : DD) { double r=org.apache.commons.math3.util.FastMath.copySign(xy[0],xy[1]); assertEquals(r, org.apache.commons.math3.util.FastMath.copySign(r,xy[1]), 1e-12); } }
    @Test public void bB_magnitude() { for (double[] xy : DD) { assertEquals(Math.abs(xy[0]), Math.abs(org.apache.commons.math3.util.FastMath.copySign(xy[0],xy[1])), 1e-12); } }
    @Test public void mM_scale_homog() { for (double[] xy : DD) { for (double c : new double[]{2.0,3.0}) assertEquals(c*Math.abs(xy[0]), Math.abs(org.apache.commons.math3.util.FastMath.copySign(c*xy[0],xy[1])), 1e-9+1e-9*c*Math.abs(xy[0])); } }
}