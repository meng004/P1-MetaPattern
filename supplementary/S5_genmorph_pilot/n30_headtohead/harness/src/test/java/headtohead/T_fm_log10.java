...
/** Auto-generated head-to-head MR class for SUT fm_log10 (org.apache.commons.math3.util.FastMath.log10). Blocks: Lstar,Ole. */
public class T_fm_log10 {
    ...
    @Test public void nN_mullog() { for (double[] xy : DDP) { double a=xy[0],b=xy[1]; if (a<=0||b<=0) continue; double lhs=org.apache.commons.math3.util.FastMath.log10(a*b), rhs=org.apache.commons.math3.util.FastMath.log10(a)+org.apache.commons.math3.util.FastMath.log10(b); assertEquals(rhs, lhs, 1e-9*Math.abs(rhs)+1e-12); } }
    @Test public void nN_powers() { for (int k=0;k<8;k++){ assertEquals((double)k, org.apache.commons.math3.util.FastMath.log10(org.apache.commons.math3.util.FastMath.pow(10.0,(double)k)), 1e-9); } }
    @Test public void nN_one() { assertEquals(0.0, org.apache.commons.math3.util.FastMath.log10(1.0), 1e-15); }
    @Test public void bB_one() { assertEquals(0.0, org.apache.commons.math3.util.FastMath.log10(1.0), 1e-15); }
    @Test public void mM_scale_homog() { for (double[] xy : DDP) { double a=xy[0],b=xy[1]; if (a<=0||b<=0) continue; double lhs=org.apache.commons.math3.util.FastMath.log10(a*b), rhs=org.apache.commons.math3.util.FastMath.log10(a)+org.apache.commons.math3.util.FastMath.log10(b); assertEquals(rhs, lhs, 1e-9*Math.abs(rhs)+1e-12); } }
}