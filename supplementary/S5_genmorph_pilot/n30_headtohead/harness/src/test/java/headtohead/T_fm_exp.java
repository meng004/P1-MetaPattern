...
/** Auto-generated head-to-head MR class for SUT fm_exp (org.apache.commons.math3.util.FastMath.exp). Blocks: Lstar,Ole. */
public class T_fm_exp {
    ...
    @Test public void nN_addmul() { for (double[] xy : DDs) { double a=xy[0],b=xy[1]; if (Math.abs(a)>20||Math.abs(b)>20) continue; double lhs=org.apache.commons.math3.util.FastMath.exp(a+b), rhs=org.apache.commons.math3.util.FastMath.exp(a)*org.apache.commons.math3.util.FastMath.exp(b); assertEquals(rhs, lhs, 1e-9*Math.abs(rhs)+1e-12); } }
    @Test public void nN_zero() { assertEquals(1.0, org.apache.commons.math3.util.FastMath.exp(0.0), 1e-15); }
    @Test public void nN_mono() { for (double[] xy : DDs) { double a=xy[0]; if (Math.abs(a)>20) continue; assertTrue(org.apache.commons.math3.util.FastMath.exp(a) <= org.apache.commons.math3.util.FastMath.exp(a+0.5)); } }
    @Test public void nN_recip() { for (double x : DSE) { if (Math.abs(x)>20) continue; assertEquals(1.0/org.apache.commons.math3.util.FastMath.exp(x), org.apache.commons.math3.util.FastMath.exp(-x), 1e-9*org.apache.commons.math3.util.FastMath.exp(-x)+1e-12); } }
    @Test public void bB_zero() { assertEquals(1.0, org.apache.commons.math3.util.FastMath.exp(0.0), 1e-15); }
    @Test public void mM_scale_homog() { for (double[] xy : DDs) { double a=xy[0],b=xy[1]; if (Math.abs(a)>20||Math.abs(b)>20) continue; double lhs=org.apache.commons.math3.util.FastMath.exp(a+b), rhs=org.apache.commons.math3.util.FastMath.exp(a)*org.apache.commons.math3.util.FastMath.exp(b); assertEquals(rhs, lhs, 1e-9*Math.abs(rhs)+1e-12); } }
}