...
/** Auto-generated head-to-head MR class for SUT au_gcd_long (org.apache.commons.math3.util.ArithmeticUtils.gcd). Blocks: G,Lstar,Ole,Istar. */
public class T_au_gcd_long {
    ...
    @Test public void nN_perm() { for (long[] xy : LL) { assertEquals(org.apache.commons.math3.util.ArithmeticUtils.gcd(xy[0],xy[1]), org.apache.commons.math3.util.ArithmeticUtils.gcd(xy[1],xy[0])); } }
    @Test public void nN_scale() { for (long[] xy : LL) { long p=xy[0],q=xy[1]; if (Math.abs(p)>100000L||Math.abs(q)>100000L) continue; for (long k : new long[]{2,3,5}) assertEquals(k*org.apache.commons.math3.util.ArithmeticUtils.gcd(p,q), org.apache.commons.math3.util.ArithmeticUtils.gcd(k*p,k*q)); } }
    @Test public void nN_mono() { for (long[] xy : LL) { long p=xy[0],q=xy[1]; if (p==0||q==0) continue; if (p==Long.MIN_VALUE||q==Long.MIN_VALUE) continue; long g=org.apache.commons.math3.util.ArithmeticUtils.gcd(p,q); assertTrue(g<=Math.abs(p) && g<=Math.abs(q)); } }
    @Test public void nN_eqref() { for (long[] xy : LL) { long p=xy[0],q=xy[1]; if (p==0||Math.abs(p)>100000L) continue; for (long k : new long[]{1,2,-1}) { long s=q+k*p; assertEquals(org.apache.commons.math3.util.ArithmeticUtils.gcd(p,q), org.apache.commons.math3.util.ArithmeticUtils.gcd(p,s)); } } }
    @Test public void bB_perm() { for (long[] xy : LL) { assertEquals(org.apache.commons.math3.util.ArithmeticUtils.gcd(xy[0],xy[1]), org.apache.commons.math3.util.ArithmeticUtils.gcd(xy[1],xy[0])); } }
    @Test public void mM_perm_inv() { for (long[] xy : LL) { assertEquals(org.apache.commons.math3.util.ArithmeticUtils.gcd(xy[0],xy[1]), org.apache.commons.math3.util.ArithmeticUtils.gcd(xy[1],xy[0])); } }
    @Test public void mM_sign_inv() { for (long[] xy : LL) { long p=xy[0],q=xy[1]; if (p==Long.MIN_VALUE||q==Long.MIN_VALUE) continue; assertEquals(org.apache.commons.math3.util.ArithmeticUtils.gcd(p,q), org.apache.commons.math3.util.ArithmeticUtils.gcd(-p,-q)); } }
}