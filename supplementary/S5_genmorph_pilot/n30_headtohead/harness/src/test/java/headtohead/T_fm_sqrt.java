...
/** Auto-generated head-to-head MR class for SUT fm_sqrt (org.apache.commons.math3.util.FastMath.sqrt). Blocks: Lstar,Ole. */
public class T_fm_sqrt {
    ...
    @Test public void nN_mulsplit() { for (double[] xy : DDP) { double a=xy[0],b=xy[1]; if (a<0||b<0) continue; double lhs=org.apache.commons.math3.util.FastMath.sqrt(a*b), rhs=org.apache.commons.math3.util.FastMath.sqrt(a)*org.apache.commons.math3.util.FastMath.sqrt(b); assertEquals(rhs, lhs, 1e-9*Math.abs(rhs)+1e-12); } }
    @Test public void nN_square() { for (double x : DSP) { if (x<0||x>1e7) continue; assertEquals(x, org.apache.commons.math3.util.FastMath.sqrt(x*x), 1e-9*x+1e-9); } }
    @Test public void nN_mono() { for (double x : DSP) { if (x<0) continue; assertTrue(org.apache.commons.math3.util.FastMath.sqrt(x) <= org.apache.commons.math3.util.FastMath.sqrt(x+1.0)); } }
    @Test public void bB_square() { for (double x : DSP) { if (x<0||x>1e7) continue; assertEquals(x, org.apache.commons.math3.util.FastMath.sqrt(x*x), 1e-9*x+1e-9); } }
    @Test public void mM_scale_homog() { for (double[] xy : DDP) { double a=xy[0],b=xy[1]; if (a<0||b<0) continue; double lhs=org.apache.commons.math3.util.FastMath.sqrt(a*b), rhs=org.apache.commons.math3.util.FastMath.sqrt(a)*org.apache.commons.math3.util.FastMath.sqrt(b); assertEquals(rhs, lhs, 1e-9*Math.abs(rhs)+1e-12); } }
}