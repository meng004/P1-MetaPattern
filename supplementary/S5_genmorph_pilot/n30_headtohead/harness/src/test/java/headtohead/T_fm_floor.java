...
/** Auto-generated head-to-head MR class for SUT fm_floor (org.apache.commons.math3.util.FastMath.floor). Blocks: Tstar,Istar,Ole. */
public class T_fm_floor {
    ...
    @Test public void nN_intshift() { for (double x : DSF) { for (int k : new int[]{1,2,-3}) assertEquals(org.apache.commons.math3.util.FastMath.floor(x)+k, org.apache.commons.math3.util.FastMath.floor(x+k), 0.0); } }
    @Test public void nN_idem() { for (double x : DSF) { assertEquals(org.apache.commons.math3.util.FastMath.floor(x), org.apache.commons.math3.util.FastMath.floor(org.apache.commons.math3.util.FastMath.floor(x)), 0.0); } }
    @Test public void nN_le() { for (double x : DSF) { if (Double.isInfinite(x)||Double.isNaN(x)) continue; assertTrue(org.apache.commons.math3.util.FastMath.floor(x) <= x); } }
    @Test public void nN_mono() { for (double x : DSF) { assertTrue(org.apache.commons.math3.util.FastMath.floor(x) <= org.apache.commons.math3.util.FastMath.floor(x+1.0)); } }
    @Test public void bB_le() { for (double x : DSF) { if (Double.isInfinite(x)||Double.isNaN(x)) continue; assertTrue(org.apache.commons.math3.util.FastMath.floor(x) <= x); } }
    @Test public void mM_additive() { for (double x : DSF) { for (int k : new int[]{1,2,-3}) assertEquals(org.apache.commons.math3.util.FastMath.floor(x)+k, org.apache.commons.math3.util.FastMath.floor(x+k), 0.0); } }
}