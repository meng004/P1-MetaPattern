...
/** Auto-generated head-to-head MR class for SUT fm_max_d (org.apache.commons.math3.util.FastMath.max). Blocks: G,Tstar,Istar. */
public class T_fm_max_d {
    ...
    @Test public void nN_commute() { for (double[] xy : DD) { assertEquals(org.apache.commons.math3.util.FastMath.max(xy[0],xy[1]), org.apache.commons.math3.util.FastMath.max(xy[1],xy[0]), 0.0); } }
    @Test public void nN_idem() { for (double x : DS) { assertEquals(x, org.apache.commons.math3.util.FastMath.max(x,x), 0.0); } }
    @Test public void nN_shift() { for (double[] xy : DD) { double c=2.5; assertEquals(org.apache.commons.math3.util.FastMath.max(xy[0],xy[1])+c, org.apache.commons.math3.util.FastMath.max(xy[0]+c,xy[1]+c), 1e-9); } }
    @Test public void nN_ge() { for (double[] xy : DD) { double m=org.apache.commons.math3.util.FastMath.max(xy[0],xy[1]); assertTrue(m>=xy[0]-1e-12 && m>=xy[1]-1e-12); } }
    @Test public void bB_commute() { for (double[] xy : DD) { assertEquals(org.apache.commons.math3.util.FastMath.max(xy[0],xy[1]), org.apache.commons.math3.util.FastMath.max(xy[1],xy[0]), 0.0); } }
    @Test public void mM_perm_inv() { for (double[] xy : DD) { assertEquals(org.apache.commons.math3.util.FastMath.max(xy[0],xy[1]), org.apache.commons.math3.util.FastMath.max(xy[1],xy[0]), 0.0); } }
    @Test public void mM_additive() { for (double[] xy : DD) { double c=2.5; assertEquals(org.apache.commons.math3.util.FastMath.max(xy[0],xy[1])+c, org.apache.commons.math3.util.FastMath.max(xy[0]+c,xy[1]+c), 1e-9); } }
}