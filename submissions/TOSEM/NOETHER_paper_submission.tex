...
%% TOSEM is single-blind: reviewers see author names and affiliations.
%% Do not pass the acmart anonymous option.
...
\begin{enumerate}[leftmargin=*,nosep]
  ...
  \item[(d)] Elimination of induction from MetaPattern discovery. Induction is \emph{relocated} from MR-instance level to structural-decomposition level, not eliminated.
\end{enumerate}
\end{tcolorbox}
...
Ying et al.'s family-tree formalism~\cite{Ying2025MRPatterns} is the closest published cousin to NOETHER's MetaPattern equivalence-class structure: both organise MR patterns into a hierarchy with explicit relationships between patterns. The two formalisms differ in the relationship type. Ying et al.'s family tree is a \emph{refinement/specialisation tree} rooted in informally-named pattern categories (symmetry, additive, multiplicative, etc.); NOETHER's MetaPattern equivalence classes are \emph{quotients of the algebra-induced MR space} $\mathrm{MR}(\mathcal{A}_P)$ under structural equivalence (Definition~\ref{def:alg-induced}), with each class derived from a specific structural component of $\mathcal{D}(\mathcal{A}_P)$. A family-tree node in Ying et al.\ typically corresponds to one or more NOETHER MetaPatterns when the node admits an operator-algebraic specification (e.g.\ Ying et al.'s ``symmetry'' parent node decomposes into the $G$ MetaPattern $m_{\mathrm{inv}}$ for finite-group symmetries plus the $T^{*}$ MetaPattern $m_{\mathrm{adj}}$ for self-adjoint dualities under NOETHER's structural decomposition); conversely, NOETHER's $\mathcal{B}^{*}_{\mathrm{rel}}$ MetaPattern has no direct counterpart in the family-tree formalism because relational-algebra equivalences were not in Ying et al.'s benchmark set. The two formalisms are complementary: Ying et al.\ catalogues patterns inductively at the MR-instance level; NOETHER provides an algebraic warrant for the existence of each pattern, given the operator algebra. A unified family-tree-plus-structural-component reading of the literature would map each family-tree node to (at most) one NOETHER structural component when the node has an algebraic source, and would flag family-tree nodes without algebraic source as candidates for the additional-MetaPattern list of Remark~\ref{rem:counterex}.
...
Such catalogues are useful, but each is, by construction, an empirical artefact. None of the existing catalogues, including prior ones from the same research line, can answer the three foundational questions of Section~\ref{sec:intro}.
...
\caption{The five MetaPatterns of the decomposition $\mathcal{D}(\mathcal{A}_P)$, with the two refinements ($\mathcal{D}^{*}$, $\mathcal{E}^{*}$) and the relational extension ($\mathcal{B}^{*}_{\mathrm{rel}}$),
each shown with the structural invariant that gives rise to its MR class. The
symmetry ($G$), self-adjoint ($T^{*}$), and time-reversal
($\mathcal{T}^{*}_{\mathrm{rev}}$) MetaPatterns form the Lie-group / self-adjoint /
...
$T^{*}$ MetaPatterns, and $\mathcal{B}^{*}_{\mathrm{rel}}$ extends the framework beyond
the core. These structures are populated per program family and may be empty (for example
...
For each of the three operator algebras instantiated in this paper ($\mathcal{A}_{\mathrm{Boltz}}$, $\mathcal{A}_{\mathrm{equi}}$, $\mathcal{A}_{\mathrm{rel}}$), we adopt a canonical $\pi$-template per non-empty structural component under which $|\mathcal{I}_s/\sim_s|=1$ (Boltzmann's $G$ MetaPattern, for example, is treated as a single $\sim_s$ class with a $\pi$-template covering both geometric quarter-rotations and energy-group permutations). Under this instance-level grouping, $|\mathbb{M}(\mathcal{A}_P)|$ equals the number of non-empty structural components for each instance, so the empirical material that reports a fixed per-instance MetaPattern count (\S\S\ref{sec:reactor}, \ref{sec:cross-domain}, \ref{subsec:third-domain}) refers to these per-instance class counts and not to a general single-class assumption. A finer treatment (separating geometric and energy-group symmetry into two $\sim_s$ classes, for example) is permitted by Step~3 of CONSTRUCT-MP and would split the corresponding MetaPattern accordingly.
\end{remark}

\begin{remark}[Decomposition sufficiency vs.\ \texttt{Translate} sufficiency]
\label{rem:block-vs-translate}
Hypothesis~\ref{hyp:seven-blocks} asserts that the eight structural components suffice to \emph{assign} every operator in $\mathcal{A}_P$ relevant to MR derivation. It does not assert that every MR formulable over $\mathcal{A}_P$'s operators is reachable through the framework's \texttt{Translate} operator from a single component invariant. Section~\ref{subsec:negative-pwr} exhibits, on the PWR core diffusion algebra $\mathcal{A}_{\mathrm{PWR}}$, two MRs whose constituent operators are individually assigned to structural components of $\mathcal{D}(\mathcal{A}_{\mathrm{PWR}})$ (rod-insertion semigroup; boration and moderator-temperature scaling), yet whose MR content is not in $\mathrm{MR}(\mathcal{A}_{\mathrm{PWR}})$ in the sense of Definition~\ref{def:alg-induced}. The structural decomposition is therefore a necessary but not sufficient input to MetaPattern construction: \texttt{Translate}'s expressive form (single-component invariants, first-order $\pi$-template, single partial-order direction, operating on $P(x)$ tuples rather than on operator-spectrum quantities) is a second, independent constraint on the framework's reach. This distinction between ``decomposition sufficiency'' (Hypothesis~\ref{hyp:seven-blocks}) and ``\texttt{Translate} sufficiency'' (open) is made explicit in \S\ref{subsec:negative-pwr}.
\end{remark}

\begin{remark}[Known and conjectured out-of-scope program families]
\label{rem:counterex}
The hypothesis is open to refutation. Six families are known or conjectured to fall outside its image and to require an additional MetaPattern:
\begin{enumerate}
...
\item \emph{Sheaf-theoretic / categorical constructions.} Programs whose semantics are captured by functors and natural transformations between categories (certified compilers, type-driven program transformations); MRs of the form ``commutativity of a square in the underlying category'' do not reduce to single-component invariants under the present \texttt{Translate}.
...
\item \emph{Topological invariants.} MRs of the form ``$f$'s level sets have a fixed Betti-number signature'' (topological data analysis, shape classifiers) require homological structure absent from the present structural components.
...
\item \emph{Empirical parameter-distribution divergence.} A probability-distribution divergence operator on parameter-space measures, motivated by the §\ref{subsec:deepcrime-pilot} pilot's category-v-02/04/05 (activation change, bias removal, weight re-init) which the eight structural components do not detect. An MR of the form $D_{\mathrm{KL}}(p_\theta \| p_{\theta+\Delta\theta}) \le \tau$ for $\|\Delta\theta\| \le \epsilon$ would lie in such an additional MetaPattern. \emph{Empirical witness}: \S\ref{subsec:deepcrime-pilot}'s cat-v-02/04/05 mutations are not detected by any MR set including Set~N; this is that pilot's principal negative finding for Hypothesis~\ref{hyp:seven-blocks}.
\end{enumerate}
Each family signals a candidate additional MetaPattern. The framework's intended response is to admit the additional MetaPattern when the program family demands it, rather than to extend $\mathcal{B}^{*}_{\mathrm{rel}}$'s scope or to absorb the case as ``out of scope for MR testing'' in some over-broad sense. Concrete examples of MRs that lie outside Theorem~\ref{thm:closure}'s scope are catalogued in Appendix~\ref{app:out-of-scope}; one candidate additional MetaPattern (metric-stability $M_{\mathrm{lip}}$) has an explicit \texttt{Translate}-template design (Remark~\ref{rem:metric-stability-block} below).
\end{remark}

\begin{remark}[Domain-level out-of-scope]
\label{rem:domain-out-of-scope}
Beyond the candidate-additional-MetaPattern families of Remark~\ref{rem:counterex} (which signal extensions \emph{within} the framework's general programme), the following program-family classes are out of scope for NOETHER in the present form because they admit no operator-algebraic representation at all:
\begin{itemize}
...
\end{itemize}
These classes are not candidate additional MetaPatterns; they are domains where the framework's scope precondition (program family admits an explicit operator-algebraic description) is structurally absent. NOETHER's reach is therefore consistent with what its scope precondition explicitly requires: SUTs from textbook-codified mathematical, physical, or relational-algebra domains are within reach, and SUTs from general software-engineering domains without such codification are not.
\end{remark}

\begin{remark}[Metric-stability candidate additional MetaPattern, $M_{\mathrm{lip}}$]
\label{rem:metric-stability-block}
Among the candidate additional MetaPatterns of Remark~\ref{rem:counterex}, the metric-stability MetaPattern has the most concrete \texttt{Translate}-template proposal. Define $M_{\mathrm{lip}} = (\mathcal{X}, d_{\mathcal{X}}) \to (\mathcal{Y}, d_{\mathcal{Y}})$ as the family of $K$-Lipschitz maps between two metric spaces; the induced MetaPattern $m_{\mathrm{lip}}$ is the equivalence class of pointwise stability inequalities $d_{\mathcal{Y}}(P(x'), P(x)) \le K \cdot d_{\mathcal{X}}(x', x)$. The corresponding \texttt{Translate} template constructs a follow-up by metric perturbation $x' = x + \varepsilon u$ ($\|u\| = 1$, $|\varepsilon| < \delta$) and predicates a $K$-Lipschitz output bound. Canonical MetaPattern ordering would place $M_{\mathrm{lip}}$ after $\mathcal{B}^{*}_{\mathrm{rel}}$ since metric structure is independent of the eight existing component invariants (no group action on the perturbation set, no partial order on inputs, no self-adjoint operator, no parametric refinement family, no idempotent-semiring rewriting). Theorem~\ref{thm:closure}'s closure proof transfers without modification because $m_{\mathrm{lip}}$ is single-MetaPattern algebraically derived. We do not commit to $M_{\mathrm{lip}}$ as part of the canonical decomposition in this paper; we record it in Appendix~\ref{app:out-of-scope} as the most concrete sub-instance of Remark~\ref{rem:counterex} item~(iv) (topological invariants) that admits an immediate \texttt{Translate} template, and as the empirical orphan in the audit of \S\ref{subsec:reactor-mapping}.
\end{remark}
...
$\mathsf{f}_{\mathrm{mono}.\mathrm{shape}}$ dynamic shape (g, $\mathcal{D}^{*}$; alias $m_{\mathrm{dyn}}$) & $O_{\le}$ & I & $Z(P x)\le Z(x)$ (no spurious extrema) \\
$\mathsf{f}_{\mathrm{conv}.\mathrm{lim}}$ convergence (h) & $\mathcal{L}^{*}$ & I & $P_{h_2}x{\to}P_{h_1}x$; fixed-point self-consistency \\
$\mathsf{f}_{\mathrm{conv}.\mathrm{rate}}$ accuracy-order (i, $\mathcal{E}^{*}$; alias $m_{\mathrm{cmp}}$) & $\mathcal{L}^{*}$ & M & $\lVert\mathrm{err}(M_2)\rVert\le\lVert\mathrm{err}(M_1)\rVert$ \\
...
\begin{remark}[Scope of Theorem~\ref{thm:closure} for the relational extension $\mathcal{B}^{*}_{\mathrm{rel}}$]
...
\begin{remark}[Scope of Theorem~\ref{thm:decidable} for the relational extension $\mathcal{B}^{*}_{\mathrm{rel}}$]
...
Steps 1--4 of CONSTRUCT-MP yield five MetaPatterns: $m_{\mathrm{inv}}$ (invariance/equivariance), $m_{\mathrm{mono}}$ (parameter-monotonicity), $m_{\mathrm{adj}}$ (self-adjoint duality / adjoint reciprocity), $m_{\mathrm{rev}}$ (time-reversal compatibility), and $m_{\mathrm{conv}}$ (discretisation convergence), together with two MR families that refine them: $m_{\mathrm{dyn}}$ (qualitative-dynamics shape invariants, refining $m_{\mathrm{mono}}$) and $m_{\mathrm{cmp}}$ (method-comparison error-bound partial orders, refining $m_{\mathrm{conv}}$).
...
The reactor-physics MetaPattern catalogue compared against here was distilled in prior work from the standard PWR-physics literature~\cite{BellGlasstone1970, LewisMiller1993} as prior inductive work; the underlying PWR-MR corpus (supplementary~S2) is a prior catalogue from the same research line, not an external corpus drawn from an unrelated team. The relationship reported in this section is therefore best read as a test of \emph{internal vocabulary coherence}: NOETHER's structural decomposition is applied to a catalogue produced inductively within the same research line, and the question is whether the algebraic re-classification reproduces, refines, or predicts within that corpus. This is internal consistency under a uniform algebraic structure, not external transfer of tacit knowledge from an independent reactor-physics team into the framework. External-transfer evidence (applying NOETHER to a reactor-physics MR corpus authored by an independent team, such as a PARCS V\&V suite or IAEA-TECDOC-class catalogue) is committed as follow-up work in supplementary~S4 (\texttt{future\_work.md}) (item (j)). The cross-codebase commons-math pilot of \S\ref{subsec:empirical-threats} (item (b.cm)) is the analogous external-transfer test on the Java head-to-head side, at $n = 3$ SUTs and $n = 77$ mutants; the reactor-side analogue remains future work.
...
\paragraph{Step 3. Run CONSTRUCT-MP, select an invariant.} Within $G$'s symmetry MetaPattern, fix attention on the rotation invariant: $f(R \cdot \mathbf{x}) = f(\mathbf{x})$ for all $\mathbf{x}$ and $R \in \mathrm{SO}(3)$.
...
The self-adjoint MetaPattern $T^{*}$ in $\mathcal{A}_{\mathrm{equi}}$ contains the attention-kernel symmetriser $T^{*}_{\mathrm{att}}$. We give two formulations, distinguished by whether the MR runs at CI-time (production-suitable) or at debug-time (one-off scaffolding).
...
\begin{itemize}
    \item \textbf{Set N (NOETHER-derived):} $\{\rho_{\mathrm{rot}},\rho_{\mathrm{mono}},\rho_{\mathrm{train}},\rho_{\mathrm{adj}},\rho_{\mathrm{train\text{-}rev}}\}$, one per non-empty MetaPattern of $\mathcal{A}_{\mathrm{equi}}$, as itemised in the ``Coverage status, revised'' paragraph of Section~\ref{subsec:rho-rev}.
...
\begin{description}
  ...
  \item[Metrics.] Per-subject mutation-detection rate (Set $\cap$ killed mutants / total killed); real-bug-detection rate (where bug logs are available); false-positive rate on baseline (un-mutated) inputs; coverage with respect to the algebraic structural decomposition $\mathrm{coverage}_{\mathrm{NOETHER}}$.
  ...
  Three sub-claims:
  \begin{description}
    \item[H3a.1 (Detection on D1, per-MetaPattern).] On the
      algebra-disrupting stratum (D1, Definition in
      \S\ref{subsec:pit-block-matrix}), Set~N's per-MetaPattern kill rate
      is competitive with or superior to Set~G's on at least one
      operative MetaPattern within scope (per-MetaPattern comparison; aggregate
    ...
    \item[H3a.2 (Complementarity).] Set~N and Set~G exhibit
      ...
      non-zero $N$-only kill count on at least one operative MetaPattern
    ...
    \item[H3a.3 (Cost-axis).] Set~N's MR-generation cost is
      ...
      cost-axis only.
  \end{description}
  \item[Pre-registered hypothesis (H3b, vs LLM-assisted baseline).] On the structural-coverage diagnostic, $\mathrm{coverage}_{\mathrm{NOETHER}}(N) > \mathrm{coverage}_{\mathrm{NOETHER}}(L_{\mathrm{ensemble}})$, where Set~$L_{\mathrm{ensemble}}$ is the LLM-assisted representative drawn from a multi-vendor $\times$ multi-temperature LLM-MR-generation protocol that subsumes the single-sample probes of Shin~\cite{Shin2024} and Zhang et~al.~\cite{ZhangChatGPTMR2023}. The diagnostic is interpreted as a structural-prior signal (NOETHER's algebraic prior contributes structural-component coverage that prompt-based LLMs lack on $\mathcal{A}_P$), \emph{not} as a fault-detection-superiority metric. \textbf{Verdict (tested, borderline-PASS at the structural reading)}: a $2$-vendor (DeepSeek, ChatGPT) $\times$ $5$-temperature ensemble harvested $487$ LLM-proposed MRs across the $10$ \S\ref{subsec:test-design} SUTs ($100$ samples); $43.5\%$ ($212/487$) of the proposals match a Set~N structural-component template under deterministic translation, and the matchable subset reproduces Set~N's kill rate on the aligned-mutant substrate ($34/70 = 0.486$ for the ensemble union, identical to Set~N on the same $70$ mutants). The remaining $56.5\%$ explore invariants outside the $8$-structural-component frame (associativity, identity / inverse element, monotonicity on SUTs lacking an O\textsubscript{le}-MetaPattern representative); these have no in-protocol kill measurement here and constitute the structural-coverage gap. NOETHER's $\mathrm{coverage}_{\mathrm{NOETHER}}$ remains $1.00$ by construction; the LLM ensemble's structural-component-template coverage of Set~N's catalogue is $43.5\%$ on the matchable subset. A third-vendor (Anthropic Claude) replication is committed in follow-up~(d.set-l-claude) of supplementary~S4 (\texttt{future\_work.md}).
...
$\mathcal{L}^{*}$-MetaPattern result as the section's central claim. The
remaining populated sub-components --- the finite-group symmetry $G$ and
its translation sub-class $G_{\mathrm{tr}}$, together with an
out-of-frame idempotence probe ($\mathcal{I}^{*}$, not among the eight
structural components of Definition~\ref{def:alg-induced}; a candidate additional MetaPattern
...
\paragraph{$G$ MetaPattern (group, symmetry).}
...
\paragraph{$G$-MetaPattern framework boundary on recursive-normalising SUTs.}
\label{para:g-block-euclidean-boundary}
The two Euclidean-style SUTs \texttt{gcdSig} and \texttt{lcmSig}
exhibit Set~N $0/7$ on the $G$ MetaPattern (Table~\ref{tab:per-block-headtohead});
...
$G$-MetaPattern sign-flip identity before the recursive Euclidean reduction
fires. The grid-synthesised $G$-MetaPattern MR
...
prologue, so $G$-MetaPattern reasoning is orthogonal to the recursive
Euclidean algorithm's algebraic core (which is closer to a divisibility
lattice $\mathcal{D}$ MetaPattern / reversibility $\mathcal{R}$ MetaPattern than
to a pre-normalisation symmetry $G$). The $0/7$ Set~N kill rate is the
correct $G$-MetaPattern reading on these SUTs under the framework's
scope precondition, not a missed opportunity for MR refinement.

\paragraph{$\mathcal{I}^{*}$ idempotence probe (out-of-frame; candidate additional MetaPattern).}
...
eight-structural-component frame; see Remark~\ref{rem:counterex}). The qualitative
prediction (low kill rate under paired-MR DSL) holds.

The four MetaPattern predictions taken together, including the central
...
that map onto Set~N's MetaPatterns (Table~\ref{tab:rediscovery}). Set~N is
...
\paragraph{Per-MetaPattern head-to-head and complementarity (Primary, PIT arm).}
Each Set~N MR derives from exactly one NOETHER structural component, so the natural head-to-head denominator is per-MetaPattern rather than the operative-MetaPattern union. Across the eight head-to-head SUTs, PIT's stock mutator catalogue exercises three operative (sub-)MetaPatterns: $G$ (finite-group symmetry: swap / negate), $\mathcal{L}^{*}$ (scale / homogeneity), and $G_{\mathrm{tr}}$, the translation sub-class of the symmetry MetaPattern $G$ (grid shift / period). For each MetaPattern~$b$, $n_{b}$ counts mutants whose (SUT, mutator) entry in \texttt{configs/sut\_block\_overrides.json} marks MetaPattern~$b$ as broken (``$\times$''). Per-MetaPattern kill counts and the complementarity partition (both / $N$-only / $G$-only / neither) appear in Table~\ref{tab:per-block-headtohead}.

The unmapped auxiliary row counts $25$ PIT mutants from cells whose D1/D2 override table does not specify broken structural components; it is a lower-bound caveat on per-MetaPattern denominators (each unmapped mutant is D1-classified but its structural-component membership is unknown), not an additional denominator added to the $G + \mathcal{L}^{*} + G_{\mathrm{tr}}$ union. The D1 pool is $n = 52$ pre and post equivalent-mutant exclusion (Table~\ref{tab:two-stratum} row~1).

The per-MetaPattern reading distinguishes three regimes:

\begin{itemize}[nosep,leftmargin=*]
\item \textbf{$G$ MetaPattern: Set~G dominates.} Set~N kills $0/7$ of the mutants Set~G exclusively reaches; the Set~N hits concentrate on \texttt{signum} ($2/4$), while \texttt{gcdSig} and \texttt{lcmSig} contribute $0/7$ for Set~N. The Euclidean-style SUTs are structurally unreachable by NOETHER's $G$-MetaPattern construction on this grid-synthesised catalogue: \texttt{gcdSig} normalises its inputs via \texttt{a < 0 ? -a : a} before the recursive \texttt{gcd(a,b)=gcd(b, a\,mod\,b)} fires, absorbing the sign-flip invariant; \texttt{lcmSig} inherits the same prologue. Whether this calls for an $\mathcal{R}$-MetaPattern re-synthesis or for documenting recursive-normalising SUTs as $G$-orthogonal is committed as follow-up~(e.4).

\item \textbf{$\mathcal{L}^{*}$ MetaPattern: complementary coverage.} Both Sets kill 8 mutants in common, 2 are $N$-only (\texttt{exactLog2}), 8 are $G$-only, 6 are jointly missed; the union covers $18/24 = 75\%$, above either Set's individual rate. Set~N's exclusive reach on \texttt{exactLog2} quantifies the coverage-extension witness of \S\ref{subsec:convergent-witnesses} per-MetaPattern; Set~G's exclusive reach on \texttt{gcdSig}+\texttt{lcmSig} compensates for the $G$-MetaPattern gap.

\item \textbf{$G_{\mathrm{tr}}$ translation sub-class: Set~N edge (underpowered).} Of $17$ translation-violating mutants, Set~N kills $10$ and Set~G kills $8$ ($N$-only $3$, $G$-only $1$, both $7$, neither $6$); union $11/17 = 64.7\%$. Set~N is $+11.7$~pp in rate and $+2$ in exclusive count on its own sub-class, but the Wilson intervals $[0.360,\,0.784]$ and $[0.262,\,0.690]$ overlap substantially. We report the per-MetaPattern edge as directional, consistent with the $G_{\mathrm{tr}}$ design prediction rather than inferential at $\alpha = 0.05$.
\end{itemize}

\paragraph{Coverage of the remaining five operative structural components.}
\label{para:augmented-stratum-pointer}
PIT 1.7.4's stock mutator catalogue does not exercise the remaining
five operative structural components ($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$,
$\mathcal{D}^{*}$, $\mathcal{E}^{*}$,
$\mathcal{B}^{*}_{\mathrm{rel}}$) on the
\S\ref{subsec:test-design} substrate. To exercise the pipeline
end-to-end on these structural components, we hand-crafted $5$ mutants per
uncovered structural component ($25$ total) that violate the targeted invariant of
a specific known Set~N MR for each structural component. These mutants are
...
\paragraph{What the per-MetaPattern reading supports, and what it does not.}
The framework's empirical contribution on the present substrate is
\emph{MetaPattern-dependent}. On the three PIT-covered operative MetaPatterns
the reading is targeted per-MetaPattern: $G_{\mathrm{tr}}$ shows the
...
reading is therefore \emph{MetaPattern-targeted precision plus
complementarity, with documented per-MetaPattern design gaps on the
...
collapses into the per-MetaPattern profile: it is driven primarily by the
$G$-MetaPattern reading on \texttt{gcdSig} + \texttt{lcmSig} and by Set~G's
denser $\mathcal{L}^{*}$ coverage on those same SUTs.

\paragraph{Aggregate D1 head-to-head (Secondary, cross-MetaPattern, $n = 52$).}
For continuity with prior reporting conventions, the cross-MetaPattern
...
aggregate gap (which favours Set~G) collapses into the per-MetaPattern
...
$G_{\mathrm{tr}}$ edge but not enough to close the cross-MetaPattern
aggregate. The aggregate is presented as secondary because it
averages over per-MetaPattern strengths and weaknesses that the
per-MetaPattern table makes visible.
...
Set~N's per-SUT structural-component catalogue translates $212$ ($43.5\%$) into
executable JIR/JOR pairs (the remaining $56.5\%$ are either compound
multi-input properties or out-of-decomposition algebra, e.g.\ associativity,
identity / inverse elements, monotonicity on SUTs whose Set~N
catalogue lacks an O\textsubscript{le}-MetaPattern representative). On the
...
$56.5\%$, Set~L explores invariants outside the $8$-structural-component frame that
...
$G$-only, both, neither). The headline reading is per-MetaPattern
...
\textbf{H3a.1 (Detection on D1, per-MetaPattern).} Per
Table~\ref{tab:per-block-headtohead} on the pre-registered
PIT-covered substrate of three populated MetaPatterns: \emph{mixed}. On the
...
the $n = 17$ MetaPattern-level sample is underpowered for an inferential
test at $\alpha = 0.05$ (Wilson intervals overlap). On the
$\mathcal{L}^{*}$ MetaPattern Set~N achieves $10/24 = 0.417$ versus Set~G
...
/ \texttt{lcmSig} respectively. On the $G$ MetaPattern the substrate's
...
\nameref{para:g-block-euclidean-boundary}); the $G$-MetaPattern $0/7$
...
MetaPattern-level rate ($2/11$ Set~N vs $9/11$ Set~G) favours Set~G on
...
Table~\ref{tab:two-stratum}); the per-MetaPattern decomposition makes
visible that the gap is concentrated on the $G$ MetaPattern plus
$\mathcal{L}^{*}$, partially offset by Set~N's $G_{\mathrm{tr}}$
edge. Coverage of the remaining five operative structural components ($O_{\le}$,
$\mathcal{T}^{*}_{\mathrm{rev}}$, $\mathcal{D}^{*}$,
$\mathcal{E}^{*}$, $\mathcal{B}^{*}_{\mathrm{rel}}$) via
hand-crafted structural-component-targeted mutants is reported in
...
$n = 70$ across 10 SUTs is underpowered for a paired head-to-head verdict at $\alpha = 0.05$; the \S\ref{subsec:pooled-headtohead} reading is the honest disclosure that Set~N is dominated by Set~G in the aggregate D1 stratum (McNemar $p = 0.019$ on D1; $p = 0.0043$ pooled), with the framework's contribution read at the per-MetaPattern, cost-axis, and D2-prediction layers rather than aggregate fault-detection superiority. The $\mathcal{L}^{*}$-blindness test on $n = 44$ across 6 SUTs is per-SUT (one third or more on more than one SUT), not pooled.
...
prediction confirmed: $\mathcal{L}^{*}$-MetaPattern blindness on
...
\begin{itemize}[nosep,leftmargin=*]
  \item Other per-MetaPattern patterns (the $G$ symmetry and $G_{\mathrm{tr}}$ translation sub-classes, plus the out-of-frame $\mathcal{I}^{*}$ probe;
  ...
  \item Head-to-head against GenMorph
        (\S\ref{subsec:pooled-headtohead}) reads per-MetaPattern, not as a
        ...
        Set~N $0/7$ on \texttt{gcdSig}~+~\texttt{lcmSig}, a $G$-MetaPattern
        ...
        the per-MetaPattern reading is the appropriate substrate-level
        contribution metric.
\end{itemize}

The full per-SUT outcome matrix, the per-MetaPattern kill table for all 30
...
\paragraph{Worked example: METRIC$+$ categories for a sorting library, mapped onto NOETHER MetaPatterns.}
\label{para:metricplus-sorting}
For a numerical comparison-sort library, METRIC$+$'s eleven input-domain $\times$ output-relation category pairs (the $9$-category METRIC base extended by $2$ output-domain pairs in METRIC$+$, per~\cite{SunMETRICplus2021} Table~II) collapse onto two NOETHER MetaPatterns: $G$ (comparator-permutation symmetry $\mathfrak{S}_n$) and $O_{\le}$ (partial order). the cell-by-cell mapping (supplementary~S8) records $9$ of $11$ category pairs mapping to $G$ or $O_{\le}$; the remaining $2$ ($D_5$ additive shifts, $R_4$ multiplicative scaling) are out-of-scope by construction for a comparison sort whose semantics ignore order-preserving univariate input transformations. The $11 \to 2$-MetaPattern compression operationalises NOETHER's deflationary direction: a NOETHER-aware METRIC$+$ user can prune the $9$ category-pair enumerations to two algebraically distinct MetaPatterns ($m_{\mathrm{inv}}$ via $G$, $m_{\mathrm{mono}}$ via $O_{\le}$) without loss of MR coverage. This is a qualitative-plus-quantitative coverage contrast, not a head-to-head fault-detection comparison: METRIC$+$ is a category-enumeration scaffold without an automated identification pipeline, so kill-rate head-to-head requires re-implementing METRIC$+$ from prose (committed as follow-up~(i) of supplementary~S4).
...
Set-MP MR; classify the structural component of each non-vacuous Set-MP MR
under $\mathcal{D}(\mathcal{A}_P)$; and identify which Set-N MR (if any)
covers the same algebraic content. The per-cell analysis (supplementary~S8) records, across the three SUTs, $6$, $5$, and $3$ non-vacuous Set-MP MRs respectively, each mapping to a NOETHER structural component also covered by Set-N's $5$ algebra-derived MRs, while three Set-N MRs ($m_{\mathrm{adj}}$, $m_{\mathrm{train\text{-}rev}}$, $m_{\mathrm{conv}}$) have no METRIC$+$ D$\times$R counterpart. The full PIT-based head-to-head kill-rate
comparison is committed as supplementary~S4 item~(i); the
present manual analysis is the structural-component mapping component of
...
structural-component coverage on the bivariate-input subsample of the \S\ref{subsec:test-design}
substrate: every non-vacuous Set-MP MR maps to a NOETHER structural component already
covered by Set-N's algebra-derived MRs, and three Set-N MetaPatterns
...
structural-component mapping of that contrast on three representative SUTs.
...
subject's category-choice specifications yields the per-structural-component
non-emptiness verdicts summarised in supplementary~S8: all four subjects are in scope via a non-empty order ($O_{\le}$) MetaPattern (with partial $G$/$\mathcal{L}^{*}$ regimes), at NOETHER MetaPattern cardinality $2$--$3$ per subject against METRIC$+$ instance-MR cardinality $142$--$3152$ (Sun et~al.\ 2021 Table~17), a $2$--$3$ orders-of-magnitude compression that the analysis below reads as complementary rather than competitive.
...
least the $O_{\le}$ MetaPattern (monotonicity in usage / weight / mileage /
...
primary instantiation domains: 2--3 of 8 structural components per subject yields
2--3 MetaPatterns, contrasted with 5--7 of 8 structural components on Boltzmann
...
classes at the structural-component level; the present analysis establishes
the structural-component coverage condition that every NOETHER structural component on
these subjects ($G$, $O_{\le}$, $\mathcal{L}^{*}$) maps to a
METRIC$+$ D$\times$R category pair, while three NOETHER structural components that
...
for partial \tildemark{} structural components, is in supplementary~S8
...
this corpus, with five structural components ($T^{*}, \mathcal{T}^{*}_{\mathrm{rev}},
...
without altering NOETHER's $8$-structural-component MetaPattern bound). Three
...
($p = 0.0044$, Set~N exclusive reach driven by $G$-MetaPattern special-status
...
\noindent \emph{NOETHER re-decomposition.} The operator algebra $\mathcal{A}_{\mathrm{sort}}$ for a deterministic comparison sort has: a permutation-symmetry MetaPattern $G \ni \mathfrak{S}_n$ (the input--output relation is permutation-equivariant in the comparator-permutation sense), a partial-order MetaPattern $O_{\le}$ (sorting respects the input order on already-sorted sub-arrays), an empty self-adjoint MetaPattern $T^{*}$ (no reciprocity structure), an empty time-reversal MetaPattern $\mathcal{T}^{*}_{\mathrm{rev}}$ (irreversible), an empty limit MetaPattern $\mathcal{L}^{*}$ (sort is exact, not asymptotic), an empty qualitative-dynamics refinement $\mathcal{D}^{*}$, and an empty method-comparison refinement $\mathcal{E}^{*}$ \emph{within a single algorithm} (and non-empty if the user is comparing multiple sorting algorithms). Therefore $\mathbb{M}(\mathcal{A}_{\mathrm{sort}}) = \{\,m_{\mathrm{inv}},\, m_{\mathrm{mono}}\,\}$, of size 2.
...
For the fault class $\Theta = \mathbb{R}^{N \times N}$ ($N = 8$), the defect $E_s(L)$ is linear in $L$ and Faithfulness reduces to $\operatorname{rank}(J_{\mathrm{test}}) = \operatorname{rank}(J_{\mathrm{full}})$. The check is performed on three symmetry instances and on the self-adjoint MetaPattern, with analytic kernel dimensions for cross-validation:
...
\end{itemize}
Faithfulness holds in every case at the analytic kernel dimension, confirming the Reachability Lemma (Lemma~\ref{lem:reachability}) numerically for $\{G, T^{*}\}$. The per-MetaPattern classification confirms that the remaining MetaPatterns ($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$, $\mathcal{L}^{*}$) fall outside the linear-equality regime of Theorem~\ref{thm:ibt} (cone, matrix-inverse, and norm-ratio nonlinearity respectively); only the sufficient direction applies to those MetaPatterns.
...
E1 is a deterministic linear-algebra check on the fault class $\Theta = \mathbb{R}^{N \times N}$, not a statistical test; it certifies that the per-generator witness sets used in our MR designs are unisolvent for the symmetry and self-adjoint MetaPatterns at $N = 8$. E2 is paired and pre-registered across the three SUTs in supplementary~S10. E3 is a sensitivity sweep rather than a hypothesis test. The framework's structural claim is the kernel characterization of Theorem~\ref{thm:ibt} (\S\ref{subsec:ibt}); E1--E3 are consistent with the theorem and with its three corollaries (\S\ref{subsec:ibt}), within the linear operator-implementation fault class (Remark~\ref{rem:ibt-scope}, R1).
...
\paragraph{Construct validity.} The reactor-physics mapping (\S\ref{subsec:reactor-mapping}) provides preliminary evidence; non-physics-domain evidence remains future work. The case study (\S\ref{subsec:case-study}) reports cat-(iv) detection $5/5$ for Set~N because the mutation set was constructed to cover one defect category per non-empty MetaPattern of $\mathcal{A}_{\mathrm{equi}}$; this exhibits construct validity of $\rho_{\mathrm{train\text{-}rev}}$ as a gradient-reversal probe rather than averaged superiority, with the DeepCrime~\cite{Humbatova2021DeepCrime} real-fault protocol committed as comparative addition. \emph{Set~N's 30 MRs were derived by a single author following CONSTRUCT-MP's four-step procedure}; an LRCA multi-LLM second-rater protocol (DeepSeek + ChatGPT + Anthropic Claude Opus) validates the author's Set~N structural-component labels: the LLM-majority label agrees with the author at Cohen's $\kappa = 0.931$ ($n = 36$, $34/36$ agreements), with per-rater agreement against the author at Cohen's $\kappa = 0.927/0.927/0.929$ (Opus/ChatGPT/DeepSeek, $n = 34/34/35$), all in the almost-perfect Landis--Koch band; the two disagreements are \texttt{L\_idem\_at\_one} (author $\mathcal{L}^{*}$, majority $T^{*}$) and \texttt{B\_rel\_xor\_reverse} (author $\mathcal{B}^{*}_{\mathrm{rel}}$, majority $\mathcal{T}^{*}_{\mathrm{rev}}$). Per-rater labels and the majority tabulation are in supplementary~S3 (\texttt{lrca\_audit.md}). A human-pair $\kappa$ replication is committed for follow-up work. Two residual construct-validity threats are disclosed here. First, the inter-rater agreement figures ($\kappa = 0.857$ on the supplementary-MR audit and, for the LRCA Set~N audit, $\kappa = 0.931$ for the LLM-majority against the author) are drawn from rater panels composed entirely of large language models (DeepSeek, ChatGPT, Anthropic Claude Opus); because these models share substantial pre-training corpora, the agreement should be read as consistency among similarly-trained but non-coordinated raters rather than as independent human-level verification; the $\kappa$ values cannot be interpreted as equivalent to inter-rater reliability from independent human domain experts, and a human inter-rater $\kappa$ study is committed as follow-up rather than claimed in this paper. Second, the four Java subjects (\texttt{SPHONE}, \texttt{SBAGGAGE}, \texttt{SEXPENSE}, \texttt{SMEAL}) used in the Path~A head-to-head (\S\ref{para:path-a-results}) are re-implementations written from Sun 2021's prose specification by the same author who designed the NOETHER framework and CONSTRUCT-MP procedure; this confounds framework-design knowledge with subject-implementation fidelity and constitutes a construct-validity threat that an independent re-implementation would resolve. For the held-out industrial corpus this subject-side confound is instead absent: the SACOS, SPARK, and LOCUST codes were developed by reactor-physics engineering teams distinct from the present study's contributors, who acted as an independent third-party verification-and-validation party rather than as the codes' developers, so for these subjects implementation fidelity is not entangled with framework-design knowledge. The residual threat there is narrower and shared with the case study, namely that the relations were still identified and structural-component-labelled by the framework's designers and the codes are concentrated in the reactor-physics domain; the committed independent human inter-rater $\kappa$ study is designed to control the labelling component, and cross-domain breadth is addressed separately under external validity below.

\paragraph{External validity.} Two distinct external-validity questions are in scope: (i) whether the framework's \emph{algebraic} reach (Hypothesis~\ref{hyp:seven-blocks}) covers all program families admitting an operator algebra, and (ii) whether the \S\ref{subsec:test-design} \emph{empirical} substrate generalises across codebases within the framework's scope precondition. On (i), Hypothesis~\ref{hyp:seven-blocks} covers a wide swathe of mathematical structure but is not exhaustive; Remark~\ref{rem:counterex} catalogues six candidate additional-MetaPattern program-family classes (symplectic, sheaf-theoretic, probabilistic / martingale, topological, label-consistency, empirical-parameter-distribution), of which two have explicit \texttt{Translate}-template designs already proposed (metric-stability $M_{\mathrm{lip}}$ at Remark~\ref{rem:metric-stability-block}, and empirical-parameter-distribution divergence motivated by the \S\ref{subsec:deepcrime-pilot} pilot's three undetected mutations) as the most actionable extension targets. On (ii), the 10 SUTs of \S\ref{subsec:test-design} are concentrated on a single codebase (\texttt{MathSignalClass} + \texttt{ComplexSignal}) selected by the pre-registered scope criterion; they satisfy the framework's scope precondition (each admits at least one non-empty NOETHER structural component beyond $G$), so the substrate confirms applicability within scope rather than tests the framework outside its design intent. A cross-codebase pilot on Apache Commons Math 3.6.1 (3 SUTs, 5 Set~N MRs, 77 PIT 1.7.4 mutants) corroborates the within-scope generalisation direction: pooled Set~N $10/77 = 13.0\%$ Wilson 95\% CI $[7.2\%, 22.3\%]$; $G$-MetaPattern $6/21 = 28.6\%$; the framework's D2 stratum prediction passes at $2/29 = 6.9\%$. The $n$ is underpowered for $\alpha = 0.05$ hypothesis testing; full numbers, per-mutant kill matrix, and the $\mathcal{L}^{*}$-orthogonality analysis on bilinear SUTs are in supplementary~S4 (\texttt{future\_work.md} item (b.cm)). The Maven-resolved head-to-head has since been realised at scale: on $36$ Apache Commons Math 3.6.1 methods (each admitting a non-empty NOETHER structural component beyond $G$) and $836$ real PIT mutants, Set~N detects $313/836 = 37.4\%$ (Wilson 95\% CI $[34.2\%, 40.8\%]$) versus $18.4\%$ for the generic literature catalogue (Set~B) and $18.1\%$ for the METRIC+ category scaffold (Set~M); the pooled paired comparison is well-powered (discordant $b + c = 159$ and $162$), with McNemar exact $p < 10^{-47}$, risk difference $+0.19$ ($[0.16, 0.22]$), paired odds ratio $> 300$, significant after Bonferroni in both the D1 ($721$ mutants) and D2 ($115$) strata, and zero false positives ($208$ MR tests pass on the unmutated library). This is positive, adequately-powered effectiveness evidence, stated with two scope qualifiers rather than hidden: (i)~Set~N strictly subsumes both baselines (zero baseline-only kills) because the generic and METRIC+ relations are a subset of the algebra-derived set, so the $+19$ percentage-point gap quantifies the \emph{identification} advantage (NOETHER surfaces MRs the generic catalogues lack) rather than an equal-relation oracle contest; and (ii)~the genetic-programming competitor GenMorph remains the priority comparator but could be evolved on only $2$ pilot SUTs in this environment, where it is genuinely complementary (Set~G $17/25$ vs Set~N $5/25$ on \texttt{gcd}; Set~N $11/26$ vs Set~G $2/26$ on \texttt{sin}), so the search-based head-to-head at scale, the LLM-set arm, and the full D4J extension remain committed follow-up~(b). Numbers, the per-mutant kill matrix, and the full statistics are in supplementary~S5 (\texttt{n30\_headtohead/}). The replication tests scope-internal generalisation, not whether the framework should apply to programs lacking explicit operator-algebraic structure (such programs are out of scope by construction). A complementary multi-domain applicability corroboration (\S\ref{para:real-bug-protocol}) extends this within-scope evidence from one Java codebase to four third-party scientific-computing libraries (SciPy, PySCF, OpenMC, DeepXDE), populating 8 of the 10 MR families with in-the-wild faults within those four domains (the remaining two families, time-reversal and dynamic-shape, are witnessed in two additional libraries, diffrax and Clawpack/PyClaw, whose structure carries them, so all ten families have an in-the-wild witness); it carries three threats we state rather than paper over. First, the 21 faults were located retrospectively by repository archaeology, one representative per family, which risks optional stopping; this is mitigated, not eliminated, by deriving each MR a priori from the governing-equation algebra and by reporting five evidenced negative results that record where the framework predicts and finds no fault. Second, 6 of the 21 are crash-type and would also be surfaced by a generic smoke test, so the family-specific discriminating power rests on the $\approx 13$ non-crash numeric and convergence violations. Third, at $n = 21$ the corpus is descriptive rather than confirmatory, and the family labelling was performed by the authors, with an independent human inter-rater study committed as future work.

\paragraph{Out-of-construction transferability (held-out).} To test transferability beyond the domains used to curate the structural decomposition, the frozen structural components were applied without re-fitting to program families outside the construction set. (i) \emph{Industrial reactor codes.} The 110 expert-approved metamorphic relations of three production codes, BAMBOO-C's SPARK (core depletion) and LOCUST (lattice), and the SACOS sub-channel thermal-hydraulics code, are subsumed, with few exceptions, by the order ($O_{\le}$) MetaPattern (Wilson 95\% interval on subsumption $[0.966, 1.000]$), with no orphan requiring an additional MetaPattern and a further 18 valid relations beyond the initial expert sets falling in the same MetaPattern. This is a code-level held-out confirmation: the order MetaPattern accounts for the overwhelming majority and corroborates transferability of $O_{\le}$ rather than of the full decomposition. LOCUST additionally contains a small number of relations whose structure falls outside the order MetaPattern, including the guard-conditioned MTC-versus-boron relation that instantiates the $\rho_{\mathrm{MTC\text{-}bor}}$ obstruction of \S\ref{subsec:negative-pwr}. (ii) \emph{Non-physics libraries.} On dense numerical linear algebra (tested on \texttt{numpy.linalg}) and digital signal processing (tested on \texttt{numpy.fft}), both outside the curation set, the algebra-derived relations populate five structural components each ($G$, $T^{*}$, $O_{\le}$, $\mathcal{L}^{*}$, $\mathcal{E}^{*}$, with the conservation invariant a second $G$-MetaPattern MR class rather than a distinct structural component), every relation confirmed executably on the independent library, and the two FA-rank-tight MetaPatterns behave as Theorem~\ref{thm:closure}'s companion Invariance-Blindness result predicts (the permutation/orthogonal $G$ MetaPattern and the self-adjoint $T^{*}$ MetaPattern attain the tight kernel under a finite generating set). The two legs are complementary: the industrial corpus supplies expert-validated breadth predominantly on the order MetaPattern in domain, while the library instantiations exercise the multi-component structure and the tight MetaPatterns out of domain. Corpora, derivations, and executable checks are in supplementary material \texttt{S11\_n5\_industrial} and \texttt{S12\_n5\_crossdomain}.

\paragraph{Expert monotonicity bias and the coverage argument.} A regularity across the three production codes is that the large majority of the 110 expert-approved relations are order-MetaPattern (monotone sensitivity) relations: increasing one input moves an output in a fixed direction. The principal exceptions are in LOCUST, whose guard-conditioned MTC-versus-boron relations (\S\ref{subsec:negative-pwr}) and burnup-sensitivity relations fall outside the order MetaPattern. This is consistent with how MR identification proceeds in practice, from parameter-sensitivity intuition, which is exactly the order MetaPattern; the symmetry, self-adjoint, conservation, limit, and method-comparison structural components require explicit operator-algebraic recognition and are systematically under-sampled. Yet the governing equations of these codes~\cite{LamarshBaratta2001, Stacey2007} contain precisely those structures: SACOS solves two-fluid mass, momentum, and energy conservation; LOCUST defines its homogenisation by reaction-rate conservation and its lattice by a geometric symmetry group; SPARK is a $k$-eigenvalue neutron balance with a critical-boron fixed point. NOETHER derives the corresponding conservation, reciprocity, and symmetry relations directly from those equations, without expert input (supplementary \texttt{S11}). This paragraph supports the coverage claim only: it does not claim that the additional structural components are, on average, more fault-revealing than expert MRs.
...
\paragraph{Audit guidance for infinite-group truncation.} When the symmetry MetaPattern $G$ is a finitely generated infinite discrete group under truncation parameter $K$ (e.g.\ $\mathbb{Z}^{d}$ for an unbounded grid solver), Theorem~\ref{thm:closure}'s closure result is over the truncated algebra rather than the full one. We recommend a \emph{$K$-sweep audit}: compute $\rho_{\iota,G}$ at three truncation levels $K \in \{K_0,\, 2K_0,\, 4K_0\}$ and pass if detection-rate stability is within $\pm 5\%$ across the three values (the mesh-convergence stability margin of~\cite[\S6.2]{LewisMiller1993}). The audit need not be re-run on every CI invocation; once per architectural change is sufficient. The reference implementation (supplementary~S1) includes a \texttt{k\_sweep\_audit()} helper.

\paragraph{When to check Hypothesis~\ref{hyp:seven-blocks}'s sufficiency at deployment time.} For program families outside the three instantiated domains (Boltzmann reactor physics, equivariant ML, relational query optimisers), users should explicitly enumerate the candidate operators of $\mathcal{A}_P$ and check whether each falls into one of the eight structural components. Operators that resist classification are signals that an additional MetaPattern may be required and should be reported as candidates for an extension of the structural-component taxonomy rather than absorbed into the existing taxonomy.
...
given a structural decomposition (Theorem~\ref{thm:closure}); (ii)~polynomial-time
...
in a public archival repository. The camera-ready version will additionally record the final SHA-256 hash of the
...
pipeline, is archived in a public repository whose DOI is withheld here for
review-stage metadata finalisation and restored at camera-ready.
...
newer archival version DOI if the replication package is updated during review.
...
\bibliography{NOETHER_paper_submission}
