...
- **稀缺块**:scipy Trev\* 在 pip 可复现范围稀缺;scipy G 由 fht(170f9e69a/gh-21661, 1.14.1→1.15.0)**边际**填补(scipy 自带 test_gh_21661,信号 edge-dominated 7.2e16 量级,干净候选仍稀缺);scipy O≤ 已由 Akima 两点线性(ef7437afc)升级为 in-the-wild;pyscf T\* Fock-Hermitian 构造保证(需 int-DM 边界)、pyscf O≤ 占据/密度/变分界构造保证(负结果,git 考古 8 候选全排除,见 `results/NEGATIVE_pyscf_o_le.md`);DeepXDE T\* 由 forward-mode Hessian(46e2c2e)填补但 **△ reachability**(非 public 默认路径)。
