...
| Trev\* 时间反演 | ✗ 未找到 pip 可复现候选(已确认稀缺:scipy 无 symplectic/leapfrog 积分器;唯一 backward 候选 d620670a5 为 2018 v1.2.0 first_step ENH+BUG,非可逆性不变量违反) | ✗ rt-TDDFT 唯一酉传播子已于 v2.0.0 移出主仓,BOMD velocity-Verlet 构造可逆(负结果,Verlet 实测 forward→reverse→初值 6.66e-16 HELD,见 NEGATIVE_pyscf_trev.md) | ✗ 无可逆动力学基底(负结果,git 考古 0 命中,见 NEGATIVE_openmc_trev.md) | ✗ 无时间步进积分器(PINN 全时空残差最小化,time 仅作配点坐标;负结果,见 NEGATIVE_deepxde_trev.md) |
...
- **openmc Trev\* 稀缺(确认)**:Monte Carlo 中子输运求稳态/本征(无可逆时间步进),depletion 解 Bateman 方程(严格耗散前向 ODE,CRAM,无负时间步);git 考古(corrected `-i -E`)time-reversal/reversib/symplectic/detailed-balance 全 0 命中,源码 grep 0 文件。唯一 adjoint 是 IFP 伴随权重(T\* 块,已由 bug_openmc_ifp_adjoint 覆盖),非时间可逆。见 NEGATIVE_openmc_trev.md。
- **pyscf Trev\* 稀缺(确认)**:唯一真正酉时间传播子 rt-TDDFT(`pyscf.rt`,MMUT)已于 v2.0.0(137c23d3)移出主仓至 extended-projects,现代版本不在论文 SUT 仓内;主仓现存唯一可逆基底 BOMD velocity-Verlet 由构造时间可逆(`_next_velocity` 方程从未被改),实测 H2/RHF forward→反转动量→backward 回到初值 6.66e-16(MR HELD,无可触发 bug)。见 NEGATIVE_pyscf_trev.md。
- **DeepXDE Trev\* 稀缺(确认)**:DeepXDE **根本无时间步进积分器**(源码 grep leapfrog/verlet/symplectic/odeint/rk4 = 0 文件),时间相关 PDE 由 `TimePDE`/`GeometryXTime` 把 t 当普通配点坐标做全时空残差最小化,无 forward→reverse→initial 可言;其暴露的可逆算子结构(Hessian 自伴)属 T\* 块(已由 bug_deepxde_forward_hessian 覆盖)。见 NEGATIVE_deepxde_trev.md。
...
n=20 论文 SUT 域 in-scope(+ 3 跨域),**underpowered for α=0.05 confirmatory**(CLAUDE.md C6)。descriptive 证据:NOETHER 块 MR 在论文 SUT 域(scipy/pyscf/openmc/DeepXDE)检出真实缺陷,覆盖 **L\*/守恒/T\*/G/O≤ 五块、四域**(pde_numerical/quantum_chemistry/reactor_physics/pde_sciml);其中 2 个 caveated(scipy fht G 信号边际、DeepXDE forward-mode Hessian T\* 非默认路径 reachability),已诚实标注;Trev\* 在**全部四个论文 SUT 域**均确认结构性稀缺(scipy 无 symplectic 积分器、openmc 无可逆动力学基底、pyscf 唯一酉传播子 rt-TDDFT 已于 v2.0.0 移出主仓且 BOMD velocity-Verlet 构造可逆、DeepXDE 无时间步进积分器),四份负结果文档(NEGATIVE_{scipy 见 §3,openmc,pyscf,deepxde}_trev)落地。
