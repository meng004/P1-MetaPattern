...
4. **剩余诚实负结果**:**Trev\* 全四域确认结构性稀缺**(scipy 无 symplectic 积分器、openmc 无可逆动力学基底、pyscf 唯一酉传播子 rt-TDDFT 已于 v2.0.0 移出主仓 + BOMD velocity-Verlet 构造可逆实测 6.66e-16 HELD、DeepXDE 无时间步进积分器;四份 `NEGATIVE_*_trev.md`);pyscf **T\* Fock-Hermitian + O≤ 占据/密度/变分界**(均构造保证,`NEGATIVE_pyscf_o_le.md`);scipy G 干净 order-of-magnitude 候选(fht 边际填补后,非边际候选仍稀缺)。**Trev\* 是唯一在全部四个论文 SUT 域均为结构性负结果的块——因四域核心都不是可逆动力学模拟器(稳态求解/本征/不动点 SCF/残差最小化 PINN)。**
