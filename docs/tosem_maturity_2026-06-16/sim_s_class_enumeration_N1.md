# N1 — 逐域 ∼_s 等价类枚举(证伪 Option C 一般性;支持 Option B 一般性 + 实例单类)

> **二次更新(2026-06-17,作者拍板)**:最终采纳 **Option B**(每等价类一 MP)。
> 三域**在实例层面采单一 $\sim_s$ 类的 $\pi$-template**(见正文 `rem:single-class-instances`),
> 使每域 $|\mathbb{M}|=$ 非空块数(保住已发表 "7 MP" 与 $\kappa=0.857$ 审计);
> N1 的"某块 ≥2 个 $\sim_s$ 类"证据现作 **B 一般性的支撑**(框架允许多类,实例选单类),
> 同时仍证伪 **C 的一般单类假设**。下文"采纳 A"段以本更新为准。
