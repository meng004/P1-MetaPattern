...
This paper introduces \textbf{NOETHER}, a two-layer framework for MetaPattern discovery. The \emph{downstream layer} is mechanical and provable: given an operator-algebra block decomposition, the \texttt{CONSTRUCT-MP} algorithm produces a MetaPattern set with an algebraic-closure guarantee under the framework's \texttt{Translate} operator over the algebra-induced MR space $\mathrm{MR}(\mathcal{A}_{P})$ (Theorem~1), and polynomial-time decidability under a finite generating set (Theorem~2). Theorem~1 converts an empirical-adequacy claim about MetaPattern coverage into a structural-adequacy obligation within an explicitly bounded scope (Definition~\ref{def:alg-induced} fixes $\mathrm{MR}(\mathcal{A}_{P})$ as the \texttt{Translate}-image of $\mathcal{A}_{P}$). The strictly stronger absolute-completeness conjecture over arbitrary properties expressible in $\mathcal{A}_{P}$ (Theorem~$1'$) is falsified on the PWR core diffusion algebra. The \emph{upstream layer} is a curated eight-block decomposition over recurrent mathematical structures (symmetry, order, self-adjoint, time-reversal, limit, qualitative-dynamics, method-comparison, and relational equivalence), stated as an explicit empirical hypothesis with a documented out-of-scope catalogue rather than as an axiom.

The framework's scope precondition is that the program family under test admits an explicit operator-algebraic description through mathematical or physical equations; programs lacking such structure (web applications without operator-algebraic semantics, RLHF reward models, distributed-consensus protocols, compiler-internal optimisations whose algebra is not in the eight blocks) are out of scope by construction. Within this scope, we instantiate NOETHER on three structurally distinct \emph{operator-algebraic} domains, testing transferability at the algebra-skeleton level rather than asserting cross-domain empirical superiority: a Boltzmann reactor-physics transport solver, where the framework systematises a prior inductive catalogue and re-classifies further equivalence classes; equivariant machine learning, where executable MRs are derived for rotation invariance, adjoint duality, and training-trajectory reversibility; and relational query optimisers, whose idempotent-semiring algebra exercises the relational-equivalence block. Structural-coverage predictions are consistent with the data within each case-study scope; the framework's central falsifiable prediction (an $\mathcal{L}^{*}$-blindness pattern on homogeneity-preserving mutators, ex-ante derivable from the algebra and the PIT mutator specification) is observed on five of six SUTs admitting an $L_{\mathrm{scale}}$ MR on the comparative substrate. Comparative evaluation against three SOTA representatives (a GP-evolved baseline, an LLM-assisted baseline, and a mining-based baseline) is reported alongside an algebra-aligned metric framework that distinguishes algebra-disrupting from algebra-preserving mutants and an MR-generation cost matrix; on the scope-matched D1 stratum, Set~N is dominated by the GP-evolved baseline (a head-to-head superiority claim is not asserted), and the framework's contribution is read as algebraic derivability, per-block complementarity, and an out-of-scope D2-stratum boundary that no inductive baseline can derive ex-ante. A negative instantiation on the PWR core diffusion algebra falsifies the absolute-completeness conjecture (Theorem~$1'$) via two independent counterexamples that identify five structural extensions of the framework's \texttt{Translate} operator, whose pairwise independence is proved by per-block exhaustion; companion structural-obstruction surveys on the equivariant-ML and relational-query algebras identify five further candidate extensions whose pairwise independence is asserted by inspection rather than by formal proof, totalling ten \texttt{Translate}-extension dimensions across the three algebras as the principal locus of follow-up work. NOETHER lifts induction from per-program MR sampling to a stable per-domain algebraic layer and makes the downstream step deductive and mechanical without eliminating induction at the upstream layer.
\end{abstract}
...
\begin{itemize}
  ...
  \item \textbf{C2b (negative theory).} The strictly stronger absolute-completeness conjecture (Theorem~$1'$ / Conjecture~\ref{conj:absolute}), over arbitrary properties expressible in $\mathcal{A}_{P}$, is \emph{false} on the PWR core diffusion algebra $\mathcal{A}_{\mathrm{PWR}}$: two specific MRs from the standard PWR safety-analysis literature (non-additivity of rod-bank reactivity worth, and second-order mixed dependence of $k_{\mathrm{eff}}$ on moderator temperature and boron concentration) are formulable over $\mathcal{A}_{\mathrm{PWR}}$ but not in $\mathrm{MR}(\mathcal{A}_{\mathrm{PWR}})$ (\S\ref{subsec:negative-pwr}, Appendix~\ref{app:negative-proofs}). The two MRs identify five structural obstructions in \texttt{Translate}'s signature (operator-spectrum output, homomorphism-failure $\pi$-template, configuration-indexed adjoint structure, higher-order mixed-difference templates, two-direction joint parametric dependence). Companion structural-obstruction surveys on $\mathcal{A}_{\mathrm{equi}}$ and $\mathcal{A}_{\mathrm{rel}}$ (\S\ref{subsec:third-domain}; \texttt{theory/equi\_thm1prime\_search.md}, \texttt{theory/rel\_thm1prime\_search.md}, \texttt{theory/translate\_extensions.md}) identify five further candidate dimensions: two on $\mathcal{A}_{\mathrm{equi}}$ (product-group orbit and gauge-bundle section, which specialise PWR-side dimensions to type-distinct algebraic primitives) and three net on $\mathcal{A}_{\mathrm{rel}}$ (aggregate-as-algebra, constraint-aware equivalence, three-valued logic), yielding ten \texttt{Translate}-extension dimensions across the three algebras as the principal locus of follow-up work. Pairwise-independence is established on the five PWR-side obstructions by per-block exhaustion in Appendix~\ref{app:negative-proofs} and asserted by inspection on the five candidate dimensions; formal per-dimension exhaustion proofs on $\mathcal{A}_{\mathrm{equi}}$ and $\mathcal{A}_{\mathrm{rel}}$ are committed as follow-up. The scope of each result is stated explicitly in Section~\ref{subsec:completeness}.
  ...
  \item \textbf{C4.} We demonstrate \emph{structural} transferability at the algebra-skeleton level (not cross-domain empirical superiority) \emph{within the framework's scope precondition} (programs admitting an explicit operator-algebraic description) by instantiating NOETHER on three structurally distinct \emph{operator-algebraic} domains: Boltzmann reactor-physics transport, equivariant machine learning (Section~\ref{sec:cross-domain}), and relational query optimisers (Section~\ref{subsec:third-domain}), the last exercising the relational-equivalence block whose algebraic skeleton differs from the Lie-group / self-adjoint / time-reversal core. On a small-scale comparative case study, a DeepCrime-style real-fault pilot ($n=5$, underpowered for $\alpha=0.05$ inferential conclusions), and a cross-codebase pilot replication on Apache Commons Math ($n=3$ SUTs, $5$ Set~N MRs, $77$ target-method PIT mutants, Set~G structurally N/A on Maven-resolved substrates and reported as derivability~+~per-block-reach pilot rather than head-to-head replication), the framework's structural-coverage prediction is consistent with the data within case-study scope; the case study's category-(iv) detection contrast exhibits construct validity of $\rho_{\mathrm{train\text{-}rev}}$ as a gradient-reversal probe rather than averaged superiority, since the mutation set was constructed to cover one defect category per non-empty block of $\mathcal{A}_{\mathrm{equi}}$. A larger comparative evaluation against three SOTA representatives (one per category: GP-evolved, LLM-assisted, mining-based) is reported as a pre-registered protocol.
\end{itemize}
...
reached by PIT 1.7.4's stock mutator catalogue, across the eight
in-scope SUTs of \S\ref{subsec:test-design}. Each row's denominator
$n_{b}$ counts mutants violating block $b$'s invariant. The
complementarity partition splits the $n_{b}$ mutants into the four
cells ``both~kill'' / ``Set~N-only kill'' / ``Set~G-only kill'' /
``neither kills''; the row total equals $n_{b}$. Wilson 95\%
intervals are reported alongside the point rates. The five
operative blocks not exercised by PIT 1.7.4
($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$,
$\mathcal{D}^{*}$, $\mathcal{E}^{*}$,
$\mathcal{B}^{*}_{\mathrm{rel}}$) are reported separately in
Appendix~\ref{app:augmented-stratum} as a construct-trace
consistency check on hand-crafted block-targeted mutants; per
\S\ref{app:augmented-stratum}, those results are design-implied by
the construction of the mutants against known Set~N MRs and are not
used as independent fault-detection evidence in this section's H3a
verdicts. Auxiliary rows: aggregate D1 (union of the three
PIT-covered operative blocks); the unmapped bucket (PIT mutants
from $(\mathrm{SUT}, \mathrm{mutator})$ cells without an override
entry, principally \texttt{RETURN\_VALS} and \texttt{MATH} on SUTs
where the table-5-reconstruction fallback assigns D1 but does not
specify broken blocks; documented as a lower-bound caveat on
per-block denominators, not a participating row).}
...
The $\mathrm{unmapped}$ auxiliary row of
Table~\ref{tab:per-block-headtohead} reports a count of $25$ PIT
mutants from (SUT, mutator) cells whose D1/D2 override table does
not specify the broken blocks; the row is a lower-bound caveat on
the per-block denominators (each unmapped mutant is D1-classified
but its block membership is unknown), \emph{not} an additional
denominator added to the $G + \mathcal{L}^{*} + \mathcal{T}^{*}$
union. The pre-equivalent-mutant-exclusion D1 pool is
$n = 52$ and the post-exclusion D1 pool is the same $52$
(Table~\ref{tab:two-stratum}, row~1); the $25$ unmapped is a
parallel count that documents incompleteness of the override
authoring, not a fourth block whose mutants we have failed to
attribute.
...
Of $17$ $\mathcal{T}^{*}$-violating mutants, Set~N kills $10$ and
Set~G kills $8$; Set~N's exclusive reach is $3$, Set~G's is $1$,
with $7$ in common (and $6$ jointly missed). The union covers
$11/17 = 64.7\%$, larger than either Set's individual rate
($58.8\%$ vs $47.1\%$). Set~N has $+11.7$~pp in rate \emph{and}
$+2$ in exclusive count on its own block. The sample size
($n = 17$ on $\mathcal{T}^{*}$) is insufficient for an
$\alpha = 0.05$ inferential test (the 95\% Wilson intervals
$[0.360,\,0.784]$ and $[0.262,\,0.690]$ overlap substantially),
so we report the per-block Set~N edge as a directional finding
...
Appendix~\ref{app:augmented-stratum} for pipeline-correctness
...
accessed through a third-party LLM API proxy) is invoked, and the final
...
post-exclusion D2 stratum ($n = 5$, after the ISSUE-010~(e.1)~v2
override extension reclassified $3$ \texttt{PrimitiveReturnsMutator}
mutants from D1 to D2 on \texttt{gcdSig} / \texttt{hypotSig} /
\texttt{lcmSig}) governs the framework prediction above. Methodological note: the progressive 2+1 voting scheme is
...
(Definition~\ref{def:alg-induced}): on the wider 23-method
...
Appendix~\ref{app:augmented-stratum} as a construct-trace
consistency check; per Appendix~\ref{app:augmented-stratum}, those
...
supplementary~S7 (\texttt{d4j/}) alongside the SUT-selection criterion, so
...
\S\ref{subsec:pooled-headtohead} reading is the honest disclosure
that Set~N is dominated by Set~G in the aggregate D1 stratum
(McNemar $p = 0.019$ on D1; $p = 0.0043$ pooled), with the
framework's contribution read at the per-block, cost-axis, and
D2-prediction layers rather than as aggregate fault-detection
superiority. The
...
(Definition~\ref{def:alg-induced}). The kill-rate gain over the
...
(GenMorph~\cite{GenMorph2024}), an LLM-assisted baseline (a
multi-vendor $\times$ multi-temperature LLM-MR-generation protocol
that subsumes the single-sample probes of
Shin~\cite{Shin2024}, ChatGPT-MR~\cite{ZhangChatGPTMR2023},
GPTMR~\cite{GPTMR2025}, and AutoMT~\cite{AutoMT2025}), and a
mining-based baseline (MR-Scout~\cite{MRScout2024}). The arms differ
in delivery completeness, and we declare the baseline-strength
asymmetry explicitly:

\begin{itemize}[nosep,leftmargin=*]
\item The GP-evolved arm is delivered in full at GenMorph's
published 30-min budget on a single random seed; multi-seed
replication for GenMorph's GP stochasticity is committed as
follow-up~(a.budget-replication) of Table~\ref{tab:future-work}.

\item The LLM-assisted arm is delivered as a $2$-vendor
(DeepSeek-V3, ChatGPT-4o-mini) $\times$ $5$-temperature ensemble
($100$ samples). The third commercial closed-vendor frontier
offering (Anthropic Claude Sonnet / Opus class) is \emph{not}
included in the $2$-vendor harvest; we do not include the strongest
contemporary closed-vendor frontier LLMs (e.g.\ GPT-4o, Claude Opus
$4.6$ / Sonnet $4.5$) and the $2$-of-$3$-vendor coverage limits
the strength of the LLM-assisted comparison. A third-vendor Claude
replication is committed as post-acceptance
follow-up~(d.set-l-claude); we report the current Set~L results as
a $2$-vendor proxy for the LLM-assisted SOTA category rather than as
the SOTA-strongest configuration.

\item The mining-based arm is reported through an adapted estimate
of MR-Scout's reach on the \S\ref{subsec:test-design} substrate,
derived from MR-Scout's published artifact reach figures rather
than a full re-execution; this is a weaker delivery than the
GP-evolved and LLM-assisted arms, and the corresponding claim is
labelled as estimate-only throughout
\S\ref{subsec:pooled-headtohead} and
Table~\ref{tab:gen-cost}. Full MR-Scout re-execution on our
substrate is committed as follow-up~(d) in
Table~\ref{tab:future-work}.

\item METRIC+ (Sun \& Chen~\cite{SunMETRICplus2021}, the
inductive-catalogue prior art most directly contrasted with
NOETHER's algebraic derivation) is \emph{not} run head-to-head: no
automated METRIC+ identification pipeline is publicly available, and
re-implementing the $9$-category MetaPattern catalogue from
prose specification would constitute substantial software work
outside this paper's scope. We instead provide a
qualitative-plus-quantitative coverage contrast in the
\S\ref{subsec:case-study} sorting worked example below
(``METRIC+ versus NOETHER on the sorting algorithm class'') and in
the per-block coverage diagnostic of
\S\ref{subsec:test-design}: NOETHER's $\mathcal{A}_P$-derived
catalogue covers the relevant blocks of the sorting algebra that
METRIC+'s $9$-category catalogue parametrises differently, but the
two approaches are not directly substitutable as head-to-head
oracles.
\end{itemize}
...
Per H3a.3, Set~N's cost-axis advantage holds when the detection-axis
reading is on the per-block primary (Table~\ref{tab:per-block-headtohead})
rather than the D1 aggregate. On the $\mathcal{T}^{*}$ block Set~N
edges Set~G with three exclusive $N$-only kills at zero per-SUT
generation cost beyond the one-time CONSTRUCT-MP derivation
($\approx 1$ hour human per SUT, amortising across SUTs sharing
$\mathcal{A}_P$); Set~G's matching kill count costs a 30-min
GAssert GP-search run per SUT ($\approx 4$ hours wall for the
eight head-to-head SUTs in parallel-4) plus the Major-mutation
$+$ JaCoCo $+$ Randoop pipeline assembly. On the $\mathcal{L}^{*}$
block the two sets are complementary ($N$-only $= 2$, $G$-only $= 8$
on a $24$-mutant denominator); Set~N's reach is reproducible
without re-running the GP search, where Set~G's would require
re-tuning the GP budget and seed if the SUT changes. The
quantitative reading: \emph{a directional Set~N edge on the
$\mathcal{T}^{*}$ block ($10/17$ vs $8/17$, $n = 17$ underpowered for
inferential confirmation, Wilson intervals $[0.360, 0.784]$ and
$[0.262, 0.690]$ overlap) and complementarity on $\mathcal{L}^{*}$
are achieved at $\approx 1$ hour amortised human generation cost per
SUT (NOETHER) versus $\approx 30$ minutes per-SUT GP search wall
time (Set~G), with NOETHER's cost amortising across new SUTs in the
same algebra family while Set~G's restarts from scratch}. The
per-block $\mathcal{T}^{*}$ edge is a directional finding consistent
with the framework's design prediction on that block; it does not
overturn Set~G's aggregate dominance on the D1 stratum, which is
acknowledged at the head of \S\ref{subsec:pooled-headtohead}. The $G$-block gap on the substrate's two
Euclidean-style SUTs (\nameref{para:g-block-euclidean-boundary})
is a framework boundary, not a cost-axis trade-off. The
cost-axis claim of H3a.3 is therefore \emph{supported}: comparable
within-scope per-block detection at lower amortised generation
cost.
...
(a.budget-replication) GenMorph GP search 30\,min $\times$ $5$ random seeds on the \S\ref{subsec:test-design} substrate, to quantify GP stochasticity for the H3a.1 / H3a.2 readings & $\approx 20$\,h Remote compute + 1\,d local aggregation & Yields a distribution over Set~G's pooled D1 kill rate (currently single-seed point estimate $37/52$) and over the per-block complementarity 4-tuples; will quantify whether the $4$ Set-N-only kills on the PIT-covered D1 stratum are robust across GP seeds or seed-dependent \\
...
(g) Construct-trace consistency check via hand-crafted block-targeted mutants on the $5$ operative blocks PIT $1.7.4$ does not exercise ($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$, $\mathcal{D}^{*}$, $\mathcal{E}^{*}$, $\mathcal{B}^{*}_{\mathrm{rel}}$); $25$ hand-crafted mutants ($5$ per block) targeting specific Set~N MRs' invariants & \textbf{Done (Appendix~\ref{app:augmented-stratum})} & Pipeline-correctness verification: \emph{$25/25$ Set~N detection is design-implied} by mutant authoring (each mutant constructed against a known Set~N MR's invariant) \emph{and is therefore not used as independent fault-detection evidence for H3a.1}. Set~G's incidental reach is $12/25 = 0.480$ with $0$ Set-G-only kills, of which $10/15$ Set-N-only contributions are from Set~G's absence-of-MR-coverage on \texttt{isSequence}. Generic-mutation independent test of H3a.1 on the five additional blocks is committed as a separate future-work item ((h) below). Construct-trace details in Appendix~\ref{app:augmented-stratum}; rationale in \texttt{docs/block\_coverage\_augmentation\_rationale.md} of the experiment repository \\
(h) Independent (non-construct-trace) test of H3a.1 on the $5$ PIT-unexercised blocks: require an automated mutation operator catalogue that exercises $O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$, $\mathcal{D}^{*}$, $\mathcal{E}^{*}$, $\mathcal{B}^{*}_{\mathrm{rel}}$ invariants without hand-authoring against Set~N's targeted invariants (e.g.\ extend PIT $1.7.4$'s mutator set with new operators derived from the algebra of the SUT, or adopt a domain-specific operator catalogue such as MAJOR's algebra-rich extension) & $\approx 1$\,month research + tooling & Would yield generic-mutation per-block kill rates for the five additional blocks on the \S\ref{subsec:test-design} substrate, completing the H3a.1 per-block evidence base \\
(i) METRIC$+$ head-to-head: re-implement the METRIC$+$ category enumeration as an automated identifier (from \cite{SunMETRICplus2021}'s prose specification), run on the \S\ref{subsec:test-design} substrate, and compare kill rate against Set~N at the same mutation budget & $\approx 2$\,weeks engineering + $\approx 30$\,min compute & Would convert the qualitative-plus-quantitative coverage contrast of \S\ref{para:metricplus-sorting} into a head-to-head fault-detection metric; currently METRIC$+$ is contrasted only as a category-enumeration scaffold, not as an executable comparator \\
...
(j) External-transfer test on an independently-authored reactor-physics MR corpus: apply NOETHER's eight-block decomposition to a PARCS V\&V suite catalogue or an IAEA-TECDOC-class catalogue authored by a team unconnected to the present authors, and report what the framework reproduces / refines / predicts / fails to reach. The \S\ref{subsec:reactor-mapping} comparison tests internal vocabulary coherence on the authors' own 84-MR corpus; external-transfer evidence is required to convert that into a generalisable systematisation claim & $\approx 1$\,month corpus-access + classification & Would yield external-team Fleiss $\kappa$ on the eight-block labelling and a per-MR reproduction/refinement/prediction count on an unrelated reactor-physics catalogue; analogous on the reactor side to the cross-codebase commons-math pilot (b.cm) on the Java side \\
\bottomrule
\end{tabular}
\end{table}
...
(\texttt{metricplus\_sun2021\_subjects/scope\_analysis.md}). The full
instance-level head-to-head with matched cardinality remains the
follow-up of Table~\ref{tab:future-work} item~(i); the scope-precondition
verdict reported here is the algebra-block component of that
follow-up, conducted on Sun et~al.~2021's published benchmark
specifications independently of the present authors' choice of
substrate.
