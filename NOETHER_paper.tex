...
\textit{$\mathcal{D}^{*}$ (augmented)}                          & 5 & 5 & $1.000$ $[0.566,\,1.000]$ & 4 & $0.800$ $[0.376,\,0.964]$ & 4 & 1 & 0 & 0 \\
\textit{$\mathcal{E}^{*}$ (augmented)}                          & 5 & 5 & $1.000$ $[0.566,\,1.000]$ & 5 & $1.000$ $[0.566,\,1.000]$ & 5 & 0 & 0 & 0 \\
\textit{$\mathcal{B}^{*}_{\mathrm{rel}}$ (augmented)}           & 5 & 5 & $1.000$ $[0.566,\,1.000]$ & 0 & $0.000$ $[0.000,\,0.434]$ & 0 & 5 & 0 & 0 \\
\midrule
\textit{D1 aggregate, PIT-covered blocks (secondary)}            & 52 & 26 & $0.500$ $[0.369,\,0.631]$ & 37 & $0.712$ $[0.577,\,0.817]$ & 22 & 4 & 15 & 11 \\
\textit{Augmented stratum aggregate (secondary)}                 & 25 & 25 & $1.000$ $[0.867,\,1.000]$ & 12 & $0.480$ $[0.300,\,0.665]$ & 12 & 13 & 0 & 0 \\
...
On the augmented stratum, Set~N reaches $25/25 = 1.000$ (Wilson 95\%
CI $[0.867,\,1.000]$) and Set~G $12/25 = 0.480$ ($[0.300,\,0.665]$),
with $13$ Set-$N$-exclusive kills and $0$ Set-$G$-exclusive
...
\item $\mathcal{D}^{*}$ ($n = 5$): Set~N $5/5 = 1.000$, Set~G
$4/5 = 0.800$; Set~N strictly better.
\item $\mathcal{E}^{*}$ ($n = 5$): Set~N $5/5$, Set~G $5/5$; tied.
\item $\mathcal{B}^{*}_{\mathrm{rel}}$ ($n = 5$): Set~N $5/5 = 1.000$,
Set~G $0/5$; the $5$ Set-$N$-only kills again reflect Set~G's
\texttt{isSequence}-coverage absence at the 30-min budget.
\end{itemize}

\noindent Methodological observation: for the clamp / signum
mutants tagged $O_{\le}$ on positions where the block-targeted MR's
\texttt{jir}/\texttt{jor} template happens to be insensitive to
the perturbation under the existing input grid, adjacent Set~N MRs
(\texttt{G\_negate\_bound\_swap}, \texttt{L\_scale}, \texttt{G\_negate})
catch the mutation instead.\footnote{The Set N catalogue covers
...
$\ge$ Set~G on every block, strictly better on four of five
($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$, $\mathcal{D}^{*}$,
$\mathcal{B}^{*}_{\mathrm{rel}}$), tied on $\mathcal{E}^{*}$,
yielding $1.000$ overall vs $0.480$ (non-overlapping Wilson 95\%
intervals; $13$ Set-$N$-exclusive kills, $0$ Set-$G$-exclusive).
...
\emph{Augmented stratum} ($n = 25$, five new blocks): Set~N $25/25
= 1.000$ vs Set~G $12/25 = 0.480$, non-overlapping Wilson 95\%
intervals; Set~N is $\ge$ Set~G on every one of the five blocks
and strictly better on four of five
($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$, $\mathcal{D}^{*}$,
$\mathcal{B}^{*}_{\mathrm{rel}}$), tied on $\mathcal{E}^{*}$,
with $13$ $N$-only and $0$ $G$-only kills.
...
(Table~\ref{tab:per-block-headtohead}) reports $13$ $N$-only kills
and $0$ $G$-only kills, $10$ of the $13$ $N$-only contributions
...
$+$ $17$ $N$-only $+$ $15$ $G$-only $= 66/77 = 85.7\%$, against
Set~G alone's $49$ ($63.6\%$) and Set~N alone's $51$ ($66.2\%$);
...
(g) Block-coverage augmentation: hand-craft $5$ semantically motivated mutants violating each of the $5$ originally-uncovered operative blocks ($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$, $\mathcal{D}^{*}$, $\mathcal{E}^{*}$, $\mathcal{B}^{*}_{\mathrm{rel}}$) on the \S\ref{subsec:test-design} substrate, augmenting the PIT-generated mutant set with $25$ new mutants whose targeted invariants the framework's MRs are designed to detect & \textbf{Done} & Per-block kill-rate table (Table~\ref{tab:per-block-headtohead}) now covers all $8$ operative blocks with $n \ge 5$ each; augmented-stratum aggregate Set~N $25/25 = 1.000$ vs Set~G $12/25 = 0.480$ strengthens H3a.1 to ``supported on augmented stratum'' and H3a.2 to ``$17$ Set-$N$-exclusive kills on the $77$-mutant aggregated stratum, union coverage $85.7\%$''; full per-mutant rationale in \texttt{docs/block\_coverage\_augmentation\_rationale.md} and per-block results in \texttt{docs/block\_coverage\_augmentation\_results.md} of the experiment repository \\
