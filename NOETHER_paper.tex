...
labelling on the \S\ref{subsec:test-design} substrate ($n = 70$
...
\paragraph{Phase~1 kill-set overlap (substrate-level bound).}
Pending the per-mutant labelling of follow-up~(e), a kill-set overlap
analysis on the head-to-head substrate ($n = 62$ pooled across the
eight SUTs) yields defensible bounds on the D1 / D2 split. Of the 62
PIT mutants, 28 are killed by both Set~N and Set~G, 4 are killed by
Set~N only, 12 are killed by Set~G only, and 18 are killed by neither.
By the framework's design, every Set~N kill is a D1 detection, so
Set~N's D1 reach is at least $32/62 = 51.6\%$ (Wilson 95\% CI:
$39.4\%, 63.6\%$) and Set~G's D1$\cup$D2 reach is $40/62 = 64.5\%$
(Wilson 95\% CI: $52.1\%, 75.3\%$); the 12 G-only kills constitute a
candidate-D2 pool with lower-bound $12/62 = 19.4\%$ (Wilson 95\% CI:
$11.4\%, 30.9\%$), and the 18 mutants that neither set reaches
(coverage $29.0\%$, Wilson 95\% CI: $19.2\%, 41.3\%$) further partition
into D2 mutants outside both sets' reach and D1 mutants both sets
missed for input-coverage reasons. The $\mathcal{L}^{*}$-only
specialisation (2/44, Table~\ref{tab:l-blindness}) is the cleanest
D2-cell estimate already on hand because the
$\langle$MATH, $\mathcal{L}^{*}\rangle$ and
$\langle$RETURN\_VALS, $\mathcal{L}^{*}\rangle$ cells of
Table~\ref{tab:pit-block} are both ``$\circ$''. Disambiguating the
G-only and neither cells per Table~\ref{tab:pit-block} requires
preserving the raw PIT \texttt{mutations.csv}, which is committed as
the next iteration of follow-up~(e).

\paragraph{Phase~2 per-mutant classification (Table~\ref{tab:pit-block} applied).}
A subsequent iteration of follow-up~(e) re-ran PIT on all eight
head-to-head SUTs with the raw \texttt{mutations.csv} preserved, then
applied Table~\ref{tab:pit-block} per-mutant to each SUT's operative
blocks (encoded in
\texttt{configs/sut\_block\_decomposition.json} of the companion
experiment repository). Of the $n = 62$ mutants, 28 are classified D1
(at least one operative block has ``$\times$'' for the mutant's
category), 0 are classified D2 (every operative block ``$\circ$''),
and 34 are classified \emph{ambiguous} (no ``$\times$'' and at least
one ``$\sim$'' on an operative block). Stratified kill rates: Set~N
on D1 is $20/28 = 0.714$ (Wilson 95\% CI: $0.529, 0.847$); Set~G on
D1 is $23/28 = 0.821$ (Wilson 95\% CI: $0.644, 0.921$); Set~N on the
ambiguous bucket is $12/34 = 0.353$ and Set~G is $17/34 = 0.500$. The
``no D2 mutants identified'' verdict is a property of the present
Table~\ref{tab:pit-block} encoding: the ``$\sim$'' cells (e.g.,
$\langle$MATH, $O_{\le}\rangle$, $\langle$INVERT\_NEGS,
$\mathcal{L}^{*}\rangle$, $\langle$NEGATE\_CONDITIONALS,
$\mathcal{T}^{*}\rangle$) require SUT-specific resolution that the
substrate-level reconstruction conservatively defers. Resolving the
``$\sim$'' cells against each SUT's actual algebraic invariants
(rather than the generic-case verdict in
Table~\ref{tab:pit-block}) is committed as follow-up~(e.1): we expect
the ambiguous bucket to partition into additional D1 and D2 mutants,
yielding a directly testable Set~N D2 kill rate. The Phase~2 result
already establishes the stronger of two complementary claims: on the
$28$ mutants whose D1 status is settled by the generic-case
Table~\ref{tab:pit-block} alone, Set~N's design-scope kill rate is
$0.714$ and Set~G's is $0.821$, with the $11$~pp gap on D1 mutants
consistent with Set~G's overall pooled lead reported in
\S\ref{subsec:pooled-headtohead}, indicating that Set~G's advantage
is not concentrated on the algebra-preserving fault subset that
Set~N is designed against by construction.
...
\subsection{Pooled head-to-head: parity at GenMorph's published budget}
...
$= 32$, Set~G $= 40$, McNemar exact two-sided $p = 0.077$. The
...
\texttt{clamp}             &  7 & 3 & 5   & 5   & $-2$ & directional only \\
\texttt{signum}            &  6 & 4 & 4   & 4   & $0$  & tie \\
\texttt{gcdSig}            &  9 & 5 & 6   & 5   & $-1$ & directional only ($+1$ mutant at 30-min) \\
\texttt{hypotSig}          &  4 & 2 & 4   & 4   & $-2$ & directional only \\
\texttt{lcmSig}            & 11 & 3 & 8   & 8   & $-5$ & directional only \\
\texttt{powerSig}          & 12 & 8 & 10  & 10  & $-2$ & directional only \\
\midrule
\textbf{pooled (head-to-head, n=62)}     &    & \textbf{32} & \textbf{40} & \textbf{39} & $-8$ & McNemar two-sided $p = 0.077$ (30-min) \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Headline pooled rates (PIT arm, $n = 62$ mutants on 8 head-to-head SUTs).}
At GenMorph's published 30-min GAssert budget (primary), Set~N's
pooled M1 kill rate is $0.516$ (Wilson 95\% CI
$[0.394,\,0.636]$) and Set~G's is $0.645$ ($[0.521,\,0.753]$);
McNemar exact two-sided $p = 0.077$ against the fixed Set~N kill
vector\footnote{If a directional pre-registration of the form
``Set~G has more discordant kills than Set~N'' is accepted, the
one-sided $p \approx 0.038$ crosses $\alpha = 0.05$. We did not
commit a directional pre-registration in advance, and we therefore
report the conservative two-sided value as primary and do not assert
directional significance here.}. At the 1-min sensitivity budget,
the rates are Set~N $0.486$ ($[0.372,\,0.600]$, computed under the
$n=70$ ten-SUT framing in which Set~G's two N/A SUTs contribute zero
kills to the denominator) and Set~G $0.557$ ($[0.441,\,0.668]$),
McNemar two-sided $p = 0.359$. The 30-min rerun changes Set~G's
per-SUT counts on only one of the eight head-to-head SUTs
(\texttt{gcdSig}, $5 \to 6$); seven of eight SUTs are unchanged, and
no SUT regresses. A $30\times$ budget increase therefore buys $+1$
mutant on a single SUT under PIT mutation; the qualitative parity
reading is preserved at the published budget.
...
head-to-head, 30-min budget), Set~N moves to $0.516$ and Set~G to
$0.645$. Both methods benefit from the algebraically richer
...
McNemar two-sided test fails to reject the null at $\alpha = 0.05$
under the published 30-min budget; and $n = 62$ is underpowered for
a paired hypothesis test in two-sided form. The supported reading is
\emph{competitive parity at GenMorph's published budget, with a
coverage-extension witness on two SUTs where Set~G is structurally
absent} (\S\ref{subsec:convergent-witnesses}). The
$\mathcal{L}^{*}$-blindness result of
\S\ref{subsec:l-blindness-confirmed} and the structural-extension
witness are unaffected by the budget change.
...
(e) D1/D2 mutant labelling on the \S\ref{subsec:test-design} substrate (\S\ref{subsec:pit-block-matrix} matrix applied per-mutant) & Phase~1 done (substrate-level bound); Phase~2 done (28 D1 / 0 D2 / 34 ambiguous; generic-case ``$\sim$'' cells deferred) & Set~N D1 kill rate 0.714 reported in \S\ref{subsec:pit-block-matrix} \\
(e.1) Resolve Table~\ref{tab:pit-block} ``$\sim$'' cells against each SUT's algebraic invariants (vs the generic-case verdict) & $\approx 1$ day human (per-SUT review of operative blocks vs each mutator's effect on the SUT's specific algebra) & Partitions the 34 ambiguous mutants into D1 / D2; gives a directly testable Set~N D2 kill rate (predicted near zero by framework design) \\
\bottomrule
\end{tabular}
\end{table}
