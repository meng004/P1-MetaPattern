...
$n = 57$. Of the remaining $57$, $55$ are classified D1 (at least
...
override-resolved $\sim$~cell) and $2$ are classified D2 (every
...
LLM-equivalent mutants happened to be in the original $7$-mutant
D2 stratum (their exclusion does not affect the D1 stratum). Stratified
kill rates: Set~N on D1 is $26/55 = 0.473$ (Wilson 95\% CI:
$0.347, 0.602$); Set~G on D1 is $40/55 = 0.727$ (Wilson 95\% CI:
$0.598, 0.827$); Set~N on D2 is $0/2 = 0.000$ (Wilson 95\% CI:
$0.000, 0.658$) and Set~G on D2 is identically $0/2 = 0.000$. The D2 verdict is the framework's
...
\textit{unmapped (lower-bound caveat)} & 29 & --- & --- & --- & --- & --- & --- & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Framework prediction on the D2 stratum (algebra-preserving, $n = 2$).}
After equivalent-mutant exclusion (see footnote at
\S\ref{para:eq-mutant-footnote} below), the $7$ originally-classified
D2 mutants reduce to $n = 2$: five of the seven were
...
On the remaining $n = 2$ D2 mutants, Set~N kill rate $= 0/2 = 0.000$
(Wilson 95\% CI $[0.000,\,0.658]$); the framework prediction
$\mathrm{kill\;rate}_{\mathrm{D2}} \le 10\%$ holds (PASS) directionally,
though the CI upper bound is wide at $n = 2$. Set~G also kills $0/2$ on
the surviving D2 stratum, indicating D2 is a structural ceiling of
algebra-grounded testing on this substrate rather than a Set~N-specific
blindness. The D2 reading is the framework's own falsifiability
commitment (\S\ref{subsec:l-blindness-confirmed}); it is not a
head-to-head metric.
...
$p = 0.0043$\footnote{The D1-only McNemar and pooled McNemar coincide
because all $7$ originally-classified D2 mutants (now $5$ excluded as
equivalent + $2$ retained as out-of-scope) are unkilled by both Set~N
...
to $0.702$ (and within the D1 scope-matched subset to $0.473$ vs
