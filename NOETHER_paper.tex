...
Table~\ref{tab:pit-block}. Of the $n = 62$ mutants on the eight
head-to-head SUTs, $55$ are classified D1 (at least one operative
block of the SUT's algebra is broken on the mutant's category by
either Table~\ref{tab:pit-block}'s generic verdict or an
override-resolved $\sim$~cell) and $7$ are classified D2 (every
operative block of the SUT's algebra is preserved by the mutant on
the SUT's input domain); the ambiguous bucket is empty after the
overrides. Stratified kill rates: Set~N on D1 is
$26/55 = 0.473$ (Wilson 95\% CI: $0.347, 0.602$); Set~G on D1 is
$40/55 = 0.727$ (Wilson 95\% CI: $0.598, 0.827$); Set~N on D2 is
$0/7 = 0.000$ (Wilson 95\% CI: $0.000, 0.354$) and Set~G on D2 is
identically $0/7 = 0.000$. The D2 verdict is the framework's
falsifiability commitment instantiated on this substrate: the
prediction $\mathrm{kill\;rate}_{\mathrm{D2}} \le 10\%$ holds (PASS);
Set~G's identical $0$ confirms that D2 is a structural ceiling of
algebra-grounded testing on this substrate rather than a Set~N-
specific weakness. On D1 (the algebra-disrupting subset Set~N is
designed against by construction), Set~G's $25.4$~pp advantage over
Set~N is in the same direction as the pooled head-to-head
(\S\ref{subsec:pooled-headtohead}) and is significant on the exact
McNemar D1-stratum test ($p = 0.0043$). The footnote on
equivalent-mutant denominator from
\S\ref{subsec:pooled-headtohead}~(p.~\pageref{para:eq-mutant-footnote})
applies identically here, by symmetric construction of the D1 and
D2 strata.
...
\subsection{Head-to-head at GenMorph's published budget: scope-matched D1 reading plus framework D2 prediction}
...
\paragraph{Scope-matched head-to-head (PIT arm, D1 stratum, $n = 55$).}
Set~N is derived from algebraic blocks; its theoretical reach is the
$\mathrm{D1}$ subset of algebra-disrupting mutants (those that violate
at least one operative block's invariant on the SUT's input domain).
On the $n=55$ D1 mutants of the eight head-to-head SUTs at the 30-min
GAssert budget, Set~N achieves $\mathrm{M1}_{\mathrm{D1}} =
26/55 = 0.473$ (Wilson 95\% CI $[0.347,\,0.602]$) and Set~G achieves
$\mathrm{M1}_{\mathrm{D1}} = 40/55 = 0.727$
($[0.598,\,0.827]$); the exact McNemar two-sided test on the D1
stratum (discordant pairs $b=18$ where only Set~G kills, $c=4$ where
only Set~N kills) yields $p = 0.0043$. Set~G significantly
outperforms Set~N on the scope-matched subset at the published
budget. The result holds notwithstanding the structural-extension
witness (\S\ref{subsec:convergent-witnesses}) on the two SUTs where
Set~G is N/A.

\paragraph{Framework prediction on the D2 stratum (algebra-preserving, $n = 7$).}
On the $n=7$ algebra-preserving mutants of the same eight SUTs, Set~N
kill rate $= 0/7 = 0.000$ (Wilson 95\% CI $[0.000,\,0.354]$); the
framework prediction $\mathrm{kill\;rate}_{\mathrm{D2}} \le 10\%$
holds (PASS). Set~G also kills $0/7$ on the D2 stratum, indicating
D2 is a structural ceiling of algebra-grounded testing on this
substrate rather than a Set~N-specific blindness. The D2 reading is
the framework's own falsifiability commitment
(\S\ref{subsec:l-blindness-confirmed}); it is not a head-to-head
metric.

\paragraph{Pooled M1 rates (auxiliary, scope-mismatched, $n = 62$).}
For completeness against prior reporting conventions, the pooled M1
rates over D1 $\cup$ D2 are: Set~N $= 0.419$ ($[0.305,\,0.543]$),
Set~G $= 0.645$ ($[0.521,\,0.753]$), McNemar two-sided
$p = 0.0043$\footnote{The D1-only McNemar and pooled McNemar
coincide here because all $7$ D2 mutants are unkilled by both Set~N
and Set~G; D2 mutants contribute only to the concordant ``both miss''
cell of the $2{\times}2$ table, leaving the discordant pairs
$(b,c) = (18,4)$ identical on both denominators.}. We report this
pooled comparison as scope-mismatched: the $7$ D2 mutants in the
denominator are not in Set~N's theoretical reach by construction,
so the pooled gap overstates Set~N's effective miss rate within its
declared scope. At the 1-min sensitivity budget, the pooled rates
are Set~N unchanged from the kill vector at the 30-min budget when
Set~G is the only variable; Set~G's pooled M1 is $0.557$
($[0.441,\,0.668]$). The 30-min rerun changes Set~G's per-SUT
counts on one of the eight head-to-head SUTs (\texttt{gcdSig},
$5 \to 6$); seven SUTs are unchanged, none regresses. A
$30{\times}$ budget increase therefore buys $+1$ Set~G mutant on a
single SUT under PIT mutation.

\paragraph{Footnote on equivalent-mutant denominator.}\label{para:eq-mutant-footnote}
The $n=62$ denominator (and the $n=55$ D1, $n=7$ D2 stratifications)
counts PIT-SURVIVED mutants without distinguishing semantically
distinct mutants from PIT false-positives (mutants whose modified
bytecode is behaviorally indistinguishable from the original on the
SUT's declared input domain). A targeted analyst review of a stratified
sample of $8$ mutants from the four highest-risk SUTs flagged
$3/8 = 37.5\%$ as candidate-equivalent (lower bound; upper bound under
``uncertain treated as candidate'' equals the lower bound here,
because the analyst returned no ``uncertain'' verdict). The numbers
reported above are therefore conservative
upper-bound miss rates on the eq-contaminated denominator; bytecode-
canonicalisation plus behavioral-suite expansion plus symmetric
manual ratification across the full $n=62$ is the standing follow-up
on the present substrate and does not affect the D2 prediction's
falsifiability commitment\footnote{The D2 kill rate is reported on
the $7$ Set-N-out-of-scope mutants. If any of the $7$ D2 mutants is
later reclassified as equivalent, both Set~N and Set~G's denominators
shrink symmetrically and the framework prediction's PASS verdict is
preserved by construction. The scope-matched D1 head-to-head is the
metric most affected by equivalent-mutant exclusion.}.
...
$0.363$; on the in-scope algebra-rich substrate ($n = 62$
head-to-head, 30-min budget), Set~N moves to $0.419$ and Set~G to
$0.645$ (and within the D1 scope-matched subset to $0.473$ vs
$0.727$). Both methods benefit from the algebraically richer
...
(e) D1 / D2 mutant labelling on the \S\ref{subsec:test-design} substrate (\S\ref{subsec:pit-block-matrix} matrix applied per-mutant with SUT-specific overrides) & \textbf{Done} & $55$ D1 / $7$ D2 / $0$ ambiguous on $n = 62$; Set~N D1 kill rate $0.473$, Set~N D2 kill rate $0.000$ reported in \S\ref{subsec:pit-block-matrix} \\
(e.2) Equivalent-mutant exclusion on the head-to-head denominator: bytecode-canonicalisation detector, behavioral-suite expansion ($10\times$ Randoop), symmetric manual ratification across $n = 62$ & $\approx 1$ day human + $\approx 30$\,min compute & Re-aggregates Set~N M1$_{\mathrm{D1}}$ on the equivalent-excluded denominator (a stratified $8$-mutant pilot flagged $3/8$ as candidate-equivalent on the four highest-risk SUTs, triggering this follow-up) \\
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item Head-to-head against GenMorph
        (\S\ref{subsec:pooled-headtohead}) shows Set~G as the
        stronger fault-detector on the scope-matched D1 stratum at
        the published budget, while the framework's falsifiable
        prediction on the D2 stratum ($\mathrm{kill\;rate} \le 10\%$)
        holds (Set~N $= 0/7$); scope-discriminance over the
        utility-method baseline is preserved.
\end{itemize}
...
\paragraph{Construct validity.} A MetaPattern's value as a testing artefact depends on whether testers experience the algebraic equivalence classes as coherent reasoning strategies. The reactor-physics mapping in Section~\ref{subsec:reactor-mapping} provides preliminary evidence; analogous evidence in non-physics domains remains future work. The case study (\S\ref{subsec:case-study}) reports cat-(iv) detection at 5/5 for Set~N because the mutation set was constructed to cover one defect category per non-empty block of $\mathcal{A}_{\mathrm{equi}}$; this exhibits construct validity of $\rho_{\mathrm{train\text{-}rev}}$ as a gradient-reversal probe rather than superiority of NOETHER on neutrally sampled defect distributions. The comparative-evaluation protocol in \S\ref{subsec:case-study} explicitly addresses this construct-validity threat by replacing hand-crafted mutations with DeepCrime real-fault operators~\cite{Humbatova2021DeepCrime}. \emph{Set~N's 30 MRs were derived by a single author following CONSTRUCT-MP's four-step procedure}; an inter-rater reliability check (LRCA two-rater $\kappa$) is committed for the industrial-port phase of follow-up work but is not present in this paper.
