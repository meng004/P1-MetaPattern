...
(i) METRIC$+$ head-to-head on Sun et al.\ 2021's 4 published subjects (\texttt{SPHONE}, \texttt{SBAGGAGE}, \texttt{SEXPENSE}, \texttt{SMEAL}); pre-registered protocol in supplementary~S8 (\texttt{protocol\_path\_a\_headtohead.md}): H$_{\mathrm{MP1}}$ coverage subsumption, H$_{\mathrm{MP2}}$ kill-rate parity, H$_{\mathrm{MP3}}$ cost-axis asymmetry, with two readings (NOETHER expanded to instance MRs at matched cardinality; METRIC$+$ contracted to algebra-block equivalence classes) & $\approx 2$\,weeks engineering + $\approx 30$\,min compute & Converts the qualitative coverage contrast of \S\ref{para:metricplus-sorting} into an inferential measured kill-rate test on Sun 2021's published corpus, addressing the "framework authors adjudicate both sides" concern by using the comparator's own substrate \\
...
(\texttt{scope\_analysis.md}). The full instance-level head-to-head
with matched cardinality on the same four subjects is committed as
\emph{pre-registered protocol} (supplementary~S8 \texttt{protocol\_path\_a\_headtohead.md},
Table~\ref{tab:future-work} item~(i)): three pre-registered
hypotheses (H$_{\mathrm{MP1}}$ coverage subsumption, H$_{\mathrm{MP2}}$
kill-rate parity, H$_{\mathrm{MP3}}$ cost-axis asymmetry); two
matched-cardinality readings (NOETHER expanded to instance MRs;
METRIC$+$ contracted to algebra-block equivalence classes); PIT
mutation substrate; pre-registered statistical analysis plan with
McNemar exact as the primary test. The scope-precondition verdict
reported here is the algebra-block component of that follow-up,
conducted on Sun et~al.~2021's published benchmark specifications
independently of the present authors' choice of substrate.
