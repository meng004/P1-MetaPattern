...
(i) METRIC$+$ head-to-head on Sun et al.\ 2021's 4 published subjects (\texttt{SPHONE}, \texttt{SBAGGAGE}, \texttt{SEXPENSE}, \texttt{SMEAL}); pre-registered protocol in supplementary~S8 (\texttt{protocol\_path\_a\_headtohead.md}): H$_{\mathrm{MP1}}$ coverage subsumption, H$_{\mathrm{MP2}}$ kill-rate parity, H$_{\mathrm{MP3}}$ cost-axis asymmetry & \textbf{Done (Java + PIT 1.7.4)} & $n=120$ PIT mutants (full \texttt{DEFAULTS} mutator catalogue), pooled Set~N $51/120 = 42.5\%$ Wilson 95\% $[34.0\%, 51.4\%]$ vs Set~MP $53/120 = 44.2\%$ $[35.6\%, 53.1\%]$, McNemar exact $p = 0.625$ (NS, parity not rejected); $92.6\%$ both-kill rate; H$_{\mathrm{MP1}}$ not falsified at Java/PIT scale; H$_{\mathrm{MP2}}$ not rejected pooled and not rejected per-subject Bonferroni; H$_{\mathrm{MP3}}$ directionally supported. Companion Python reduced-scale ($n=219$) cross-substrate replication shows the same pooled-parity conclusion. Details in supplementary~S8 \texttt{results\_path\_a\_full.md} \\
