...
specific weakness. On D1 (the algebra-disrupting subset Set~N is designed against by
construction), the cross-block aggregate gap (Set~G $40/55 = 0.727$
vs Set~N $26/55 = 0.473$; exact McNemar two-sided $p = 0.0043$)
...
edge ($8/15$ vs $7/15$). The per-block decomposition is the
...
$\mathcal{T}^{*}$ & 15 &  8 & $0.533$ $[0.301,\,0.752]$ &  7 & $0.467$ $[0.248,\,0.699]$ & 6 & 2 & 1 & 6 \\
\midrule
\textit{D1 aggregate (secondary)} & 55 & 26 & $0.473$ $[0.347,\,0.602]$ & 40 & $0.727$ $[0.598,\,0.827]$ & --- & --- & --- & --- \\
\textit{unmapped (lower-bound caveat)} & 30 & --- & --- & --- & --- & --- & --- & --- & --- \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Aggregate D1 head-to-head (Secondary, cross-block, $n = 55$).}
For continuity with prior reporting conventions, the cross-block
aggregate D1 head-to-head is: Set~N
$\mathrm{M1}_{\mathrm{D1}} = 26/55 = 0.473$ (Wilson 95\% CI
$[0.347,\,0.602]$), Set~G $\mathrm{M1}_{\mathrm{D1}} = 40/55 =
0.727$ ($[0.598,\,0.827]$); exact McNemar two-sided $p = 0.0043$
on the D1 stratum with discordant pairs $(b,c) = (18,4)$. The
...
(e.4) $G$-block MR re-synthesis (or in-scope documentation) for Euclidean-style SUTs (\texttt{gcdSig}, \texttt{lcmSig}): the grid-synthesised $G$ MR encodes a sign-flip identity that the SUT's own \texttt{a < 0 ? -a : a} normalisation prologue absorbs before the recursive Euclidean reduction fires, so $G$-block reasoning is currently vacuous on these SUTs (Table~\ref{tab:per-block-headtohead}: Set~N $0/7$, Set~G $7/7$) & $\approx 1$ day human (CONSTRUCT-MP re-derivation targeting an $\mathcal{R}$-block reversibility identity \emph{post}-normalisation, or a paper paragraph in \S\ref{subsec:l-blindness-confirmed} documenting recursive-normalising SUTs as structurally orthogonal to $G$-block reasoning) & Partitions the 7-mutant $G$-block gap into ``MR design defect'' (redesign) vs ``framework boundary'' (document); informs the framework's per-SUT scope precondition \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item Head-to-head against GenMorph
        ...
        edge ($8/15$ vs $7/15$, two exclusive kills) consistent with
