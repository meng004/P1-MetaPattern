...
Table~\ref{tab:per-block-headtohead}: \emph{partially supported}.
On the $\mathcal{T}^{*}$ block Set~N achieves $10/17 = 0.588$
versus Set~G $8/17 = 0.471$, with three $N$-only exclusive kills,
a directional edge consistent with the algebra-induced prediction;
on the $\mathcal{L}^{*}$ block Set~N achieves $10/24 = 0.417$
versus Set~G $16/24 = 0.667$, complementary on $N$-only $= 2$ and
$G$-only $= 8$; on the $G$ block on the substrate's two
Euclidean-style SUTs (\texttt{gcdSig}, \texttt{lcmSig}) the
SUTs' normalisation prologue absorbs the $G$-action before the
recursive reduction (a documented framework boundary, see
\nameref{para:g-block-euclidean-boundary}), so the $G$-block
$0/7$ Set~N reading on those two SUTs is the
framework-correct boundary verdict rather than a missed-opportunity
MR-design defect. The aggregate D1 reading ($26/52$ vs $37/52$,
McNemar $p = 0.019$) is in Set~G's direction, but is not the load-
bearing reading per H3a's per-block clause; it is reported as
...
is currently three operative blocks ($G$, $\mathcal{L}^{*}$,
$\mathcal{T}^{*}$); coverage of the remaining five operative
blocks ($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$,
$\mathcal{D}^{*}$, $\mathcal{E}^{*}$, $\mathcal{B}^{*}_{\mathrm{rel}}$)
is the subject of a committed augmentation experiment
(follow-up~(g) of Table~\ref{tab:future-work}: hand-craft
$\ge 3$ mutants per operative block on the §\ref{subsec:test-design}
substrate so that every block has statistically interpretable
per-block kill-rate data).

\textbf{H3a.2 (Complementarity).} \emph{Supported}. The D1
complementarity partition (Table~\ref{tab:two-stratum}, row 1)
reports $4$ $N$-only kills on the $n = 52$ D1 stratum (a $7.7\%$
exclusive contribution), distributed across the $\mathcal{T}^{*}$
and $\mathcal{L}^{*}$ blocks per
Table~\ref{tab:per-block-headtohead}. Each set kills mutants the
other misses; the union $N \cup G$ covers $26 + 4 + 15 = 45$ of
$52$ D1 mutants ($86.5\%$ union coverage), against Set~G alone's
$37$ ($71.2\%$) and Set~N alone's $30$ ($57.7\%$, $26$ both-killed
$+$ $4$ $N$-only).
...
(g) Block-coverage augmentation: hand-craft $\ge 3$ semantically motivated mutants violating each of the $5$ currently-uncovered operative blocks ($O_{\le}$, $\mathcal{T}^{*}_{\mathrm{rev}}$, $\mathcal{D}^{*}$, $\mathcal{E}^{*}$, $\mathcal{B}^{*}_{\mathrm{rel}}$) on the \S\ref{subsec:test-design} substrate, augmenting the PIT-generated mutant set with $\ge 15$ new mutants whose targeted invariants the framework's MRs are designed to detect & $\approx 2$\,days human (per-block mutation design + Remote PIT eval) & Yields a per-block kill-rate table covering all $8$ operative blocks with $n \ge 3$ each, strengthening the H3a.1 (per-block detection) and H3a.2 (complementarity) sub-claim verdicts to all-block coverage; current paper presents the table for $G$, $\mathcal{L}^{*}$, $\mathcal{T}^{*}$ only \\
