...
(i) METRIC$+$ head-to-head on Sun et al.\ 2021's 4 published subjects (\texttt{SPHONE}, \texttt{SBAGGAGE}, \texttt{SEXPENSE}, \texttt{SMEAL}); pre-registered protocol in supplementary~S8 (\texttt{protocol\_path\_a\_headtohead.md}): H$_{\mathrm{MP1}}$ coverage subsumption, H$_{\mathrm{MP2}}$ kill-rate parity, H$_{\mathrm{MP3}}$ cost-axis asymmetry & \textbf{Done (reduced-scale)} & $n=219$ Python-AST mutants, pooled Set~N $53/219 = 24.2\%$ Wilson 95\% $[19.0\%, 30.3\%]$ vs Set~MP $61/219 = 27.9\%$ $[22.3\%, 34.1\%]$, McNemar exact $p = 0.152$ (NS); H$_{\mathrm{MP1}}$ \textbf{falsified} on SPHONE (9 MP-only kills, 0 N-only, $p = 0.0039 <$ Bonferroni $\alpha/4 = 0.0125$); H$_{\mathrm{MP2}}$ not rejected pooled; H$_{\mathrm{MP3}}$ deferred to full-scale. Five protocol deviations from reduced scale (Python re-implementation vs Sun's Java; AST mutation vs PIT 1.7.4; no multi-LLM eq-mutant vote) documented in supplementary~S8 \texttt{results\_path\_a.md} \\
...
\paragraph{Reduced-scale Path A head-to-head results.}
\label{para:path-a-results}
Python re-implementations of the four subjects (from Sun~2021's
prose specification, Tables~7--14) were exercised under both Set~N
(NOETHER, $\{8, 8, 11, 8\}$ instance MRs across the four subjects via
CONSTRUCT-MP + Translate category-choice binding) and Set~MP
(METRIC$+$, $\{19, 12, 18, 9\}$ instance MRs via automated D$\times$R
category enumeration). Mutants were generated by a Python AST
mutation engine implementing PIT-equivalent operators
(\texttt{MATH}, \texttt{RETURN\_VALS}, \texttt{CONDITIONALS\_BOUNDARY},
\texttt{NEGATE\_CONDITIONALS}, \texttt{CONSTANT\_REPLACE}), yielding
$n = 52 + 48 + 42 + 77 = 219$ pooled mutants. The pooled head-to-head
shows Set~N at $53/219 = 24.2\%$ Wilson $95\%$ CI $[19.0\%, 30.3\%]$
vs Set~MP at $61/219 = 27.9\%$ $[22.3\%, 34.1\%]$ with McNemar exact
$p = 0.152$ (not rejecting H$_{\mathrm{MP2}}$ parity at
$\alpha = 0.05$). Per-subject Bonferroni-corrected at
$\alpha = 0.0125$ identifies SPHONE as the one subject where Set~MP
dominates ($9$ MP-only kills, $0$ N-only, $p = 0.0039$): METRIC$+$'s
(D1, R1) within-partition-equivalence MRs on SPHONE catch
tier-threshold mutants that NOETHER's active blocks ($O_{\le}$,
$\mathcal{L}^{*}$) on that subject do not constrain. SBAGGAGE,
SEXPENSE, SMEAL each show balanced or perfectly overlapping reach
($p = 1.00$ all three). H$_{\mathrm{MP1}}$ coverage-subsumption is
\textbf{falsified} on SPHONE; the falsification is in the predicted
direction of \texttt{scope\_analysis.md}'s scope-precondition verdict
(business-rule subjects activate only $\{O_{\le}, \mathcal{L}^{*}, G\}$
of the eight blocks, so METRIC$+$'s D$\times$R categories can
exercise invariants outside the active subset). H$_{\mathrm{MP3}}$
cost-axis asymmetry is deferred to the full-scale execution because
the reduced-scale METRIC$+$ enumeration intentionally limited
combinatorial growth and does not exhibit the asymptotic gap. Five
protocol deviations (Python re-implementation vs Sun's Java; AST
mutation vs PIT; no multi-LLM equivalent-mutant vote; reduced
enumeration; single executor) are recorded in
supplementary~S8 \texttt{results\_path\_a.md}; all are data-blind
deviations driven by tool availability rather than data-informed
re-design.

The Path A reduced-scale run \emph{strengthens} this section's
complementarity reading: the two frameworks have non-overlapping
strengths on this corpus (Set~MP's tier-equivalence invariants
unreachable by Set~N's active blocks; Set~N's $G$-block
permutation invariants on SBAGGAGE/SMEAL providing $8$ N-only kills
on SBAGGAGE), with pooled kill-rate parity. Full-scale results would
quantify whether this complementarity holds at Sun~2021's published
$142$--$3152$ MR cardinality and whether the cost-axis asymmetry of
H$_{\mathrm{MP3}}$ materialises at that scale.
