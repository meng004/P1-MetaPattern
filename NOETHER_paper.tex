...
McNemar two-sided test on the D1 stratum (post eq-exclusion, $n =
52$) yields $p = 0.019$ ($\alpha = 0.05$ threshold crossed at the
published 30-min budget); the pooled $n = 57$ comparison yields
$p = 0.0043$, but the pooled gap is strengthened by Set~G's $3/5$
on the D2 stratum, an out-of-scope contribution for Set~N by
construction. \textbf{H3a (scope-matched D1 non-inferiority,
pre-registered $\Delta = 0.10$) is not supported}: the observed
D1 gap is $0.712 - 0.500 = 0.212$, exceeding $\Delta$ by a factor
of $2.1\times$. The supported reading is \emph{the framework's D2
falsifiability prediction is consistent with the data (Set~N D2
kill rate $0/5 = 0.000$, Wilson 95\% CI $[0.000,\,0.434]$; the
$\le 10\%$ ceiling is within the CI but $n = 5$ is insufficient
for $\alpha = 0.05$ confirmation), and Set~G is the stronger
fault-detector within the D1 scope-matched subset on this
substrate}. The
$\mathcal{L}^{*}$-blindness result of
