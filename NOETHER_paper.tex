...
METRIC$+$ yields $9$, $8$, and $9$ non-vacuous Set-MP MRs respectively,
each of which maps to a NOETHER block also covered by Set-N's $5$
algebra-derived MRs; conversely, three Set-N MRs ($m_{\mathrm{adj}}$,
$m_{\mathrm{train\text{-}rev}}$, $m_{\mathrm{conv}}$) have no METRIC$+$
D$\times$R counterpart.} The structural reading: NOETHER's $G$/$O_{\le}$
...
\midrule
\textit{Set-MP non-vacuous} & 6 / 11 & 5 / 11 & 4 / 11 \\
